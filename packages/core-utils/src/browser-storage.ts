...
export const getStatic = (storage: any, key: any) => {
 ...
 */
export const setStatic = (storage: any, key: any, value: any) => {
  if (value === undefined) {
...
export const listenStatic = (storage: any, key: any, fn: any) => {
  window.addEventListener('storage', (e) => {
    if (e.storageArea === storage && e.key === key && e.newValue) {
...
export const getDynamic = (storage: any, key: any) => {
  const storageKey = `dynamic-storage/${key}`;
  const data: any = maybeParseJSON(storage.getItem(storageKey));
  return data?.value;
};
 ...
 * @param value the new JSON-serializable value to be stored, or `undefined` to remove the key
 */
export const setDynamic = (storage: any, key: any, value: any) => {
  const storageKey = `dynamic-storage/${key}`;
  if (value === undefined) {
...
export const listenDynamic = (storage: any, key: any, fn: any) => {
  const storageKey = `dynamic-storage/${key}`;
  window.addEventListener('storage', (e) => {
    if (e.storageArea === storage && e.key === storageKey && e.newValue) {
      const data: any = maybeParseJSON(e.newValue);
      fn(data?.value);
    } else {
      fn(undefined);
    }
...
export const staticStorageSlot = (storage: any, key: any) => {
  ...
  return { subscribe, get, set, update: (fn: any) => set(fn(get())), reset: () => set(initialValue) };
};
