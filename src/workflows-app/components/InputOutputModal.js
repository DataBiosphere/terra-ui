...
const InputOutputModal = ({ title, jsonData, sections = undefined, onDismiss, sasToken, workspaceId }) => {
  ...
  const renderTable = (tableTitle, tableData) => {
    const dataArray = tableData ? Object.keys(tableData).map((key) => [key, tableData[key]]) : [];
    // When tableData is null, we are expecting that an HTTP request is pending
    // and so will show a loading spinner while it finishes fetching.
    // When tableData is undefined, that means that the HTTP request completed but fetched no data,
          ...
          ({ width }) =>
            h(
              FlexTable,
              {
                ...
                noContentMessage: `No ${tableTitle}`,
                columns: [
                  {
  ...
  const renderedSections = sections || [{ title, jsonData }];
  const [activeTab, setActiveTab] = useState(renderedSections[0]?.title);
  const activeSection = _.find(({ title: sectionTitle }) => sectionTitle === activeTab, renderedSections) || renderedSections[0];

  const activeTable = div({ style: { margin: '1rem 0' } }, [renderTable(activeSection.title, activeSection.jsonData)]);
      ...
      renderedSections.length > 1
        ? h(
            SimpleTabBar,
            {
              ...
              tabs: _.map(({ title: sectionTitle }) => ({ key: sectionTitle, title: sectionTitle }), renderedSections),
            },
            [activeTable]
          )
        : activeTable,
