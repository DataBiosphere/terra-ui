...
const validateRequiredHasSource = (inputSource: InputSource, inputType: InputType) => {
  ...
  if (inputSource.type === 'object_builder' && inputType.type === 'struct') {
    ...
    const fieldsValidated = _.map(
      (field: StructField & ObjectBuilderField) => validateRequiredHasSource(field.source, field.field_type) === true,
...
const validateRecordLookup = (inputSource: InputSource | undefined, inputType: InputType, recordAttributes) => {
  ...
  if (inputSource.type === 'object_builder' && inputType.type === 'struct') {
    if (inputSource.fields && inputType.fields) {
      const fieldsValidated = _.map(
        (field: StructField & ObjectBuilderField) =>
...
export const convertArrayType = ({
  ...
  if (unwrappedInput.type === 'struct' && inputSource.type === 'object_builder') {
    return {
      ...
      source: {
        ...inputSource,
        fields: _.map((field: StructField & ObjectBuilderField) => ({
          name: field.name,
...
const validateLiteralInput = (inputSource: InputSource, inputType: InputType) => {
  ...
  if (inputSource.type === 'object_builder' && inputSource.fields && inputType.type === 'struct') {
    const fieldsValidated = _.map(
      (field: StructField & ObjectBuilderField) => field && validateLiteralInput(field.source, field.field_type),
