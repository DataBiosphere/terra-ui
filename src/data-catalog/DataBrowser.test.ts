...
import { datasetAccessTypes } from './data-browser-utils';
import { convertColsToSettings, convertSettingsToCols, extractCatalogFilters } from './DataBrowser';
import { TEST_DATASET_ONE, TEST_DATASET_TWO, TEST_DATASETS } from './test-datasets';
...
const expectedFilterHeaders = _.map(
  (expectedFilterSection) => expectedFilterSection.header,
...
describe('DataBrowser', () => {
  ...
  it('generates proper filter sections for datasets', () => {
    ...
    // Assert
    _.forEach(
      (expectedHeader) => expect(extractCatalogFiltersResultHeaders).toContain(expectedHeader),
  ...
  _.forEach((filterSectionAndValuesForTestDataset) => {
    it(`generates a working matchBy function for the filter for ${filterSectionAndValuesForTestDataset.header}`, () => {
      ...
      const section = _.find(
        (filterSection) => filterSection.header === filterSectionAndValuesForTestDataset.header,
        extractCatalogFiltersResult
      );
      if (section === undefined) {
        fail(`Section ${filterSectionAndValuesForTestDataset.header} did not appear as was expected`);
      } else {
        // Act
        const matchesDatasetOne = section.matchBy(
          filterSectionAndValuesForTestDataset.testSuccessfulMatch,
          filterSectionAndValuesForTestDataset.testAgainstValue
        );
        const matchesDatasetTwo = section.matchBy(
          filterSectionAndValuesForTestDataset.testFailureMatch,
          filterSectionAndValuesForTestDataset.testAgainstValue
        );
        // Assert
