...
function handleBrushMove(event, xScale, facet) {
  ...
  if (newValue > max || newValue < min || newValue2 > max || newValue2 < min || Number.isNaN(newValue) || Number.isNaN(newValue2)) {
...
function getHistogramSlider(facet, bars, histogramWidth, histogramHeight, brushSelection, xScale) {
  // return ['', null]
...
function getHistogram(facet, bars, histogramWidth, histogramHeight, brushSelection, xScale) {
  // const sliderId = `numeric-filter-histogram-slider___${facet.name}`;
  // const [sliderLeft, sliderWidth, extentStartX, extentWidth] = getSliderStyle(bars, histogramWidth)

  const [sliderHtml, brushConf] = getHistogramSlider(facet, bars, histogramWidth, histogramHeight, brushSelection, xScale);
...
function updateFacetCheckbox(facet) {
  ...
  const facetCheckbox = document.querySelector(`.igv-facet-${facetName} .igv-facet-header-checkbox`);
...
function getCategoricalFacetHtml(facet, isPartlyCollapsed = true) {
  ...
  const filters = facet.filterNames;
...
function getNumericQueryBuilder(
  facet,
  // filterNumbers,
  statistics
) {
  ...
  const andInput2 = `<span class="igv-and-input-2" style="${andInput2Style}">
  <span>and</span>
  ${input2}
<span>`;
...
function writeNumericFacetSliderBrushes(facets, brushesByNumericFacetName) {
  facets.forEach((facet) => {
    if (facet.name in brushesByNumericFacetName) {
      ...
      const min = facet.statistics.min;
      const max = facet.statistics.max;
      d3.select(selector).call(brush).call(brush.move, [min, max].map(xScale));
    }
  });
}
