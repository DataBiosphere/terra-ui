...
export const CromwellModalBase = Utils.withDisplayName('CromwellModal')(
  ({
    ...
    const attachedDataDisk = currentAttachedDataDisk(app, dataDisks)
    //
    ...
    const createCromwell = _.flow(
      ...
      await Ajax().Apps.app(googleProject, Utils.generateAppName()).create({
        ...
        appType: 'CROMWELL', namespace, bucketName, workspaceName
      })
      Ajax().Metrics.captureEvent(Events.applicationCreate, { app: 'Cromwell', ...extractWorkspaceDetails(workspace) })
      return onSuccess()
    })

    const deleteCromwell = _.flow(
      Utils.withBusyState(setLoading),
      withErrorReporting('Error deleting cromwell instance')
    )(async () => {
      await Ajax().Apps.app(app.googleProject, app.appName).delete(attachedDataDisk ? shouldDeleteDisk : false)
      Ajax().Metrics.captureEvent(Events.applicationDelete, { app: 'Cromwell', ...extractWorkspaceDetails(workspace) })
      return onSuccess()
    })

    const pauseCromwell = _.flow(
      Utils.withBusyState(setLoading),
      withErrorReporting('Error stopping Cromwell instance')
    )(async () => {
      await Ajax().Apps.app(app.googleProject, app.appName).pause()
      Ajax().Metrics.captureEvent(Events.applicationPause, { app: 'Cromwell', ...extractWorkspaceDetails(workspace) })
      return onSuccess()
    })

    const resumeCromwell = _.flow(
      Utils.withBusyState(setLoading),
      withErrorReporting('Error starting Cromwell instance')
    )(async () => {
      await Ajax().Apps.app(app.googleProject, app.appName).resume()
      Ajax().Metrics.captureEvent(Events.applicationResume, { app: 'Cromwell', ...extractWorkspaceDetails(workspace) })
      return onSuccess()
    })

    const renderActionButton = () => {
      return h(ButtonPrimary, { onClick: createCromwell }, ['Create'])
    }
          ...
          div({ style: { flex: '1', lineHeight: '1.5rem', minWidth: 0, display: 'flex' } }, [
            ...
            div([
              ...
              div({ style: { lineHeight: 1.5 } }, [
                div(['Creating a cloud environment for Cromwell takes ', span({ style: { fontWeight: 600 } }, ['8-10 minutes.'])]),
            ...
            div([
              ...
              div({ style: { lineHeight: 1.5 } }, [
                ...
                h(Link, { href: 'https://support.terra.bio/hc/en-us/articles/360050566271', ...Utils.newTabLinkProps }, [
                  'Pausing/Resuming a Cromwell instance.',
    ...
    const renderLaunchWarning = () => {
      return div({ style: computeStyles.drawerContent }, [
        h(TitleBar, {
          id: titleId,
          title: h(WarningTitle, ['Launch Cromwell']),
          hideCloseButton: isAnalysisMode,
          style: { marginBottom: '0.5rem' },
          onDismiss,
          onPrevious: !!viewMode ? () => setViewMode(undefined) : undefined
