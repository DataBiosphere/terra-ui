...
export const NewAnalysisModal = Utils.withDisplayName('NewAnalysisModal', ({
  isOpen, onDismiss, onSuccess, uploadFiles, openUploader, runtimes, apps, galaxyDataDisks, refreshRuntimes, refreshApps, refreshAnalyses, analyses,
  workspace, persistentDisks, workspace: { workspace: { namespace, bucketName } }
}) => {
  // State
  ...
  const goToNextView = (currentTool, baseViewMode = viewMode) => {
  ...
  const renderNewRuntimeModal = () => h(NewRuntimeModalBase, {
    isOpen: currentTool === tools.Jupyter.label || currentTool === tools.RStudio.label, isAnalysisMode: true,
    workspace, tool: currentTool, runtimes, persistentDisks,
    onDismiss: () => {
      resetView()
      onDismiss()
    },
  ...
  const renderNewGalaxyModal = () => h(NewGalaxyModalBase, {
    isOpen: viewMode === tools.galaxy.label, isAnalysisMode: true,
    workspace, apps, galaxyDataDisks,
    onDismiss: () => {
      setViewMode(undefined)
      onDismiss()
    },
  ...
  const renderToolButtons = () => div({
    ...
    div({
      style: styles.toolCard, onClick: () => {
        ...
        goToNextView(tools.Jupyter.label)
      }
    }, [img({ src: jupyterLogoLong, style: { ...styles.image, width: '30%' } })]),
    div({
      style: styles.toolCard, onClick: () => {
        setCurrentTool(tools.RStudio.label)
        goToNextView(tools.RStudio.label)
      }
    ...
    div({
      style: { opacity: currentApp ? 0.5 : 1, ...styles.toolCard }, onClick: () => {
        ...
        goToNextView(tools.galaxy.label)
      }, disabled: !currentApp, title: currentApp ? 'You already have a galaxy environment' : ''
    }, [img({ src: galaxyLogo, style: { ...styles.image, width: '30%' } })])
  ])
  ...
  return h(IdContainer, [titleId => h(ModalDrawer, {
    ...
    h(TitleBar, {
      ...
      titleStyles: { margin: '1.5rem 0 0 1.5rem', display: !!viewMode ? undefined : 'none' },
      width,
      onDismiss: () => {
        resetView()
        onDismiss()
    ...
    !!viewMode && hr({ style: { borderTop: '1px solid', width: '100%', color: colors.accent() } }),
    Utils.switchCase(viewMode,
      [NEW_ANALYSIS_MODE, () => {
        return div({ style: { display: 'flex', flexDirection: 'column', flex: 1, padding: '0.5rem 1.5rem 1.5rem 1.5rem' } }, [
          h2({ style: { fontWeight: 600, marginBottom: 0 } }, [`Create a new ${getArtifactLabel(currentTool)}`]),
          renderCreateAnalysisBody(currentTool)
        ])
      }],
      [NEW_ENVIRONMENT_MODE, () => currentTool === tools.galaxy.label ? renderNewGalaxyModal() : renderNewRuntimeModal()],
      [Utils.DEFAULT, () => {
        return div({
          ...
          h(Dropzone, {
            ...
            onDropAccepted: files => {
              ...
              currentRuntime && !isRuntimeDeletable(currentRuntime) && currentRuntimeTool !== tool ?
                onSuccess() :
                goToNextView(tool, NEW_ANALYSIS_MODE)
              uploadFiles()
            }
          }, [() => div({
            style: {
              marginTop: '1rem', fontSize: 16, lineHeight: '20px', ...Style.elements.card.container, alignItems: 'center', width: '100%',
              height: 150, backgroundColor: colors.dark(0.1), border: `1px dashed ${colors.dark(0.7)}`, boxShadow: 'none'
            },
            onClick: () => {
              resetView()
              onSuccess()
