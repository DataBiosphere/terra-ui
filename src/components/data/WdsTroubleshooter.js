...
export const WdsTroubleshooter = ({ onDismiss, workspaceId, mrgId }) => {
  ...
  useOnMount(() => {
    Ajax(signal)
      ...
      .then((res) => {
        ...
        Ajax(signal)
          .WorkspaceData.getVersion(foundApp?.proxyUrls?.wds)
          .then((res) => {
        ...
        Ajax(signal)
          .WorkspaceData.getStatus(foundApp?.proxyUrls?.wds)
          .then((res) => {
  ...
  const troubleShooterRow = ([label, text, iconRunning, iconSuccess, element]) => {
    return tr([
      td({ style: { fontWeight: "bold" } }, [iconRunning ? checkIcon("running") : iconSuccess ? checkIcon("success") : checkIcon("failure")]),
  ...
  const proxyElement = td([h(Fragment, [h(div, { style: _.merge({ width: "400px", float: "left" }, Style.noWrapEllipsis) }, [proxyUrl])])]);
  ...
  return h(
    ...
    [
      div({ style: { padding: "1rem 0.5rem", lineHeight: "1.4rem" } }, [
        table(
          { style: { borderSpacing: "1rem 0", borderCollapse: "separate" } }, // [
          tableRows
        ),
        h(Fragment, {}, [
