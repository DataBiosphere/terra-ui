...
> ({ value, id, findValue, ...props }: BaseSelectProps<Value, Option, IsMulti, Group>) => {
  const newValue: RSelectPropsValue<Option> = props.isMulti ? _.compact(_.map(findValue, value as Value[])) : findValue(value as Value)
  const myId = useUniqueId()
  const inputId = id || myId

  const ParameterizedRSelect = RSelect as typeof RSelect<Option, IsMulti, Group>
  return h(ParameterizedRSelect, _.merge({
    inputId,
    ...commonSelectProps,
...
> ({ value, options = [], ...props }: SelectProps<Value, IsMulti, Option>) => {
  ...
  const newOptions = (options && !_.isObject(options[0]) ? _.map(value => ({ value }), options) : options) as Option[]
