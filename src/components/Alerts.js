...
const Alert = ({ alert }) => {
  ...
  const [baseColor, ariaLabel] = Utils.switchCase(severity,
  ...
  const iconType = Utils.switchCase(severity,
  ...
  return div({
    ...
    link && div({ style: { marginTop: '0.25rem' } }, [
      h(Link, { ...Utils.newTabLinkProps, href: link, style: { fontWeight: 700 } }, [
        linkTitle || 'Read more'
      ])
    ])
  ])
...
const AlertsIndicator = ({ style }) => {
  ...
  return h(Fragment, [
    ...
    _.map(
      ({ id, title, message }) => div({ key: id, role: 'alert', className: 'sr-only' }, [
        div([title]),
        !!message && div([message])
      ]),
      alerts
