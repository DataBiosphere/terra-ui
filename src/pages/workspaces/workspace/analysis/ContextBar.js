...
import { appTools, getAppType, toolLabelTypes, tools } from 'src/pages/workspaces/workspace/analysis/tool-utils'
...
export const ContextBar = ({
  ...
  const isTerminalEnabled = currentRuntimeTool === toolLabelTypes.Jupyter && currentRuntime && currentRuntime.status !== 'Error'
  ...
  const getImgForTool = toolLabel => Utils.switchCase(toolLabel,
    [toolLabelTypes.Jupyter, () => img({ src: jupyterLogo, style: { height: 45, width: 45 }, alt: '' })],
    [toolLabelTypes.Galaxy, () => img({ src: galaxyLogo, style: { height: 40, width: 40 }, alt: '' })],
    [toolLabelTypes.Cromwell, () => img({ src: cromwellImg, style: { width: 45 }, alt: '' })],
    [toolLabelTypes.RStudio, () => img({ src: rstudioSquareLogo, style: { height: 45, width: 45 }, alt: '' })],
    [toolLabelTypes.JupyterLab, () => img({ src: jupyterLogo, style: { height: 45, width: 45 }, alt: '' })]
  )
  ...
  const getEnvironmentStatusIcons = () => {
    ...
    return h(Fragment, [
      ...
      ...(galaxyApp ? [getIconForTool(toolLabelTypes.Galaxy, galaxyApp.status)] : []),
      ...(cromwellApp ? [getIconForTool(toolLabelTypes.Cromwell, cromwellApp.status)] : [])
    ])
  }
