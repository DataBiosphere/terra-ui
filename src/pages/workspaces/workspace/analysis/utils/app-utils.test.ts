...
describe('getCurrentAppDataDisk', () => {
  it('returns undefined if no disk exists for the given app type', () => {
    expect(getCurrentAppDataDisk(appTools.GALAXY.label, [cromwellProvisioning], [cromwellProvisioningDisk], 'test-workspace')).toBeUndefined()
  })
  it('returns the newest attached disk, even if app is deleting', () => {
    expect(getCurrentAppDataDisk(appTools.GALAXY.label, mockApps, mockAppDisks, 'test-workspace')).toStrictEqual(galaxyDeletingDiskUpdatedPd)
    expect(getCurrentAppDataDisk(appTools.CROMWELL.label, mockApps, mockAppDisks, 'test-workspace')).toStrictEqual(cromwellProvisioningDiskUpdatedPd)
  })
  it('returns the newest unattached disk that is not deleting if no app instance exists', () => {
    expect(getCurrentAppDataDisk(appTools.GALAXY.label, [], mockAppDisks, 'test-workspace')).toStrictEqual(galaxyDiskUpdatedPd)
    expect(getCurrentAppDataDisk(appTools.CROMWELL.label, [galaxyRunning], mockAppDisks, 'test-workspace')).toStrictEqual(cromwellUnattachedDiskUpdatedPd)
  })
