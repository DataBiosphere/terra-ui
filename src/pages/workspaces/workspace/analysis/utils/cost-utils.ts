...
export const getDefaultIfUndefined = (input: number | undefined, alternative: number): number => input || alternative;
...
export const runtimeConfigBaseCost = (config: GoogleRuntimeConfig): number => {
  ...
  const costForDataproc: number = isDataprocConfig(config)
    ? (config.masterDiskSize +
        config.numberOfWorkers * getDefaultIfUndefined(config.workerDiskSize, defaultDataprocWorkerDiskSize)) *
        getPersistentDiskPriceForRegionHourly(computeRegion, pdTypes.standard) +
      dataprocCost(config.masterMachineType, 1) +
      dataprocCost(config.workerMachineType, config.numberOfWorkers)
    : 0;
...
export const runtimeConfigCost = (config: GoogleRuntimeConfig): number => {
  ...
  const additionalDataprocCost: number = isDataprocConfig(config)
    ? _.sum([
        ...
        getDefaultIfUndefined(config.numberOfPreemptibleWorkers, 0) *
          getHourlyCostForMachineType(config.workerMachineType, computeRegion, true),
        getDefaultIfUndefined(config.numberOfPreemptibleWorkers, 0) *
          getDefaultIfUndefined(config.workerDiskSize, 0) *
          getPersistentDiskPriceForRegionHourly(computeRegion, pdTypes.standard),
        dataprocCost(config.workerMachineType, getDefaultIfUndefined(config.numberOfPreemptibleWorkers, 0)),
        ephemeralExternalIpAddressCost({
          numStandardVms: config.numberOfWorkers,
          numPreemptibleVms: getDefaultIfUndefined(config.numberOfPreemptibleWorkers, 0),
        }),
      ])
    : 0;

  const gpuCost =
    isGceRuntimeConfig(config) && !_.isNil(config.gpuConfig)
...
export const getPersistentDiskPriceForRegionHourly = (computeRegion, diskType) =>
...
export const getRuntimeCost = ({ runtimeConfig, status, cloudContext = {} }) => {
  ...
  return Utils.switchCase(
    status,
    ["Stopped", () => runtimeConfigBaseCost(runtimeConfig)],
    ["Error", () => 0.0],
...
export const getCostDisplayForTool = (app, currentRuntime, currentRuntimeTool, toolLabel) => {
  return Utils.cond(
    [
      ...
      () => (app ? `${getComputeStatusForDisplay(app.status)} ${Utils.formatUSD(getGalaxyComputeCost(app))}/hr` : ""),
    ],
