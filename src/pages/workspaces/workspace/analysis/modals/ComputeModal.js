...
const sparkInterfaces = {
  ...
  appHistory: {
    ...
    synopsis:
      'YARN Application Timeline provides information about current and historic applications executed on the cluster.',
  },
  sparkHistory: {
    label: 'sparkhistory',
        ...
        h(Fragment, [
          ...
          div([
            h(Select, {
              ...
              value: currentMemory,
              onChange: (option) =>
                onChange(_.find({ cpu: currentCpu, memory: option.value }, machineTypeOptions)?.name || value),
              options: _.flow(
...
export const ComputeModalBase = ({
  ...
  const mainMachineType =
    _.find({ name: computeConfig.masterMachineType }, validMachineTypes)?.name ||
    getDefaultMachineType(isDataproc(runtimeType), tool);
  const machineTypeConstraints = {
    inclusion: { within: _.map('name', validMachineTypes), message: 'is not supported' },
  };

  const isRuntimeRunning = currentRuntimeDetails?.status === 'Running';
  const shouldDisplaySparkConsoleLink =
    isDataproc(runtimeType) && currentRuntimeDetails?.runtimeConfig?.componentGatewayEnabled;
  ...
  const errors = validate(
    ...
    },
    {
      prettify: (v) =>
        ({
          customEnvImage: 'Container image',
  ...
  const applyChanges = _.flow(
    ...
    const shouldUpdatePersistentDisk =
      canUpdatePersistentDisk() && !_.isEqual(desiredPersistentDisk, existingPersistentDisk);
    ...
    if (shouldUpdateRuntime || shouldCreateRuntime) {
        ...
        ...(desiredRuntime.cloudService === cloudServices.GCE
          ? {
              ...
              ...(computeConfig.gpuEnabled && {
                gpuConfig: { gpuType: computeConfig.gpuType, numOfGpus: computeConfig.numGpus },
              }),
            }
          : {
      ...
      if (shouldUpdateRuntime) {
        const updateRuntimeConfig = _.merge(
        ...
        await Ajax()
          ...
          .create({
            ...
            ...(desiredRuntime.jupyterUserScriptUri
              ? { jupyterUserScriptUri: desiredRuntime.jupyterUserScriptUri }
              : {}),
          });
      }
    }
  ...
  const getMainMachineTypeByMemory = (numCpus, memory) =>
    _.find({ cpu: numCpus, memory }, validMachineTypes)?.name || mainMachineType;
  ...
  const getValidCpuGpuConfig = (machineType) => {
    ...
    const validNumGpus = _.includes(computeConfig.numGpus, validNumGpusOptions)
      ? computeConfig.numGpus
      : _.head(validNumGpusOptions);
  ...
  const isRStudioImage =
    getToolLabelForImage(_.find({ image: selectedLeoImage }, leoImages)?.id) === runtimeTools.RStudio.label;
  ...
  const getCurrentMountDirectory = (currentRuntimeDetails) => {
    ...
    return currentRuntimeDetails?.labels.tool
      ? currentRuntimeDetails?.labels.tool === 'RStudio'
        ? rstudioMountPoint
        : jupyterMountPoint
      : noMountDirectory;
  };
      ...
      persistentDisk: Utils.cond(
        ...
        [
          ...
          () => ({
            ...
            diskType: computeConfig.selectedPersistentDiskType,
          }),
        ],
        () => existingPersistentDisk,
      ),
  ...
  const handleLearnMoreAboutPersistentDisk = () => {
    ...
    Ajax().Metrics.captureEvent(Events.aboutPersistentDiskView, {
      ...
      currentlyHasAttachedDisk: !!hasAttachedDisk(),
    });
  };
  ...
  const makeImageInfo = (style) => {
    ...
    const shouldDisable = _.isEmpty(leoImages)
      ? true
      : selectedImage.isCommunity || getToolLabelForImage(selectedImage.id) === toolLabels.RStudio;
    const changelogUrl = _.isEmpty(leoImages)
      ? ''
      : `https://github.com/DataBiosphere/terra-docker/blob/master/${_.replace(
          '_legacy',
          '',
          selectedImage.id,
        )}/CHANGELOG.md`;
  ...
  const sendCloudEnvironmentMetrics = () => {
    ...
    const desiredMachineType =
      desiredRuntime &&
      (desiredRuntime.cloudService === cloudServices.GCE
        ? desiredRuntime.machineType
        : desiredRuntime.masterMachineType);
    const existingMachineType =
      existingRuntime &&
      (existingRuntime?.cloudService === cloudServices.GCE
        ? existingRuntime.machineType
        : existingRuntime.masterMachineType);
    ...
    Ajax().Metrics.captureEvent(Events[metricsEvent], {
      ...
      desiredRuntime_pausedCostPerHour:
        desiredRuntime && runtimeConfigBaseCost(getPendingRuntimeConfig(), getPendingDisk()),
      ...
      desiredPersistentDisk_costPerMonth:
        desiredPersistentDisk && getPersistentDiskCostMonthly(getPendingDisk(), computeConfig.computeRegion),
  ...
  useOnMount(() => {
    ...
    const doUseOnMount = _.flow(
      ...
      const [currentRuntimeDetails, newLeoImages, currentPersistentDiskDetails] = await Promise.all([
        currentRuntime
          ? Ajax().Runtimes.runtime(currentRuntime.googleProject, currentRuntime.runtimeName).details()
          : null,
        Ajax()
      ...
      const filteredNewLeoImages = !!tool
        ? _.filter((image) => _.includes(image.id, runtimeTools[tool].imageIds), newLeoImages)
        : newLeoImages;

      const imageUrl = currentRuntimeDetails
        ? getImageUrl(currentRuntimeDetails)
      ...
      const autopauseThresholdCalculated = !!currentRuntimeDetails
        ? currentRuntimeDetails.autopauseThreshold
        : defaultAutopauseThreshold;
      const newRuntimeType = Utils.switchCase(
        runtimeConfig?.cloudService,
        [
          cloudServices.DATAPROC,
          () => (runtimeConfig.numberOfWorkers === 0 ? runtimeTypes.dataprocSingleNode : runtimeTypes.dataprocCluster),
        ],
      ...
      setComputeConfig({
        ...
        selectedPersistentDiskType:
          (!!currentPersistentDiskDetails?.diskType && currentPersistentDiskDetails.diskType) ||
          defaultPersistentDiskType,
    ...
    return div({ style: computeStyles.drawerContent }, [
      ...
      div({ style: { lineHeight: 1.5 } }, [
        p([
          ...
          code({ style: { fontWeight: 600 } }, [getCurrentMountDirectory(currentRuntimeDetails)]),
          br(),
  ...
  const renderActionButton = () => {
    ...
    const commonButtonProps = Utils.cond(
      ...
      [
        computeConfig.gpuEnabled &&
          _.isEmpty(getValidGpuTypesForZone(computeConfig.computeZone)) &&
      ...
      () =>
        h(
          ButtonPrimary,
          {
            ...
            tooltipSide: 'left',
            tooltip: isUpdateDisabled
              ? `Cannot perform change on environment in (${currentRuntimeDetails.status}) status`
              : 'Update Environment',
          },
          [
            Utils.cond(
      ...
      h(IdContainer, [
        (id) =>
          h(Fragment, [
            ...
            div({ style: { height: 45 } }, [
              renderImageSelect({ id, includeCustom: tool === toolLabels.Jupyter || tool === toolLabels.RStudio }),
            ]),
          ]),
      ]),
      Utils.switchCase(
          ...
          () => {
            return h(Fragment, [
              h(IdContainer, [
                (id) =>
                  h(Fragment, [
                    label({ htmlFor: id, style: { ...computeStyles.label, display: 'block', margin: '0.5rem 0' } }, [
                      'Container image',
                    ]),
                    div({ style: { height: 52 } }, [
                      h(ValidatedInput, {
              ...
              div([
                ...
                ...Utils.switchCase(
                  ...
                  [
                    ...
                    () => [
                      'the ',
                      h(Link, { href: anVILRStudioImage, ...Utils.newTabLinkProps }, ['AnVIL RStudio image']),
                    ],
                  ],
                  [
                    toolLabels.Jupyter,
                    () => [
                      'one of the ',
                      h(Link, { href: terraBaseImages, ...Utils.newTabLinkProps }, [
                        'Terra Jupyter Notebook base images',
                      ]),
                    ],
                  ],
                ),
  ...
  const renderComputeProfileSection = (computeExists) => {
    ...
    return div({ style: { ...computeStyles.whiteBoxContainer, marginTop: '1rem' } }, [
                  ...
                  h(Fragment, [
                    ...
                    div({ style: { height: 45 } }, [
                      h(Select, {
                        ...
                        value: validGpuName,
                        onChange: ({ value }) =>
                          updateComputeConfig('gpuType', _.get('type', _.find({ name: value }, validGpuOptions))),
                  ...
                  div({ style: { flex: 1, marginRight: '2rem' } }, [
                    h(Select, {
                      ...
                      onChange: ({ value }) => {
                        ...
                        const defaultMachineTypeForSelectedValue = getDefaultMachineType(
                          isDataproc(value),
                          getToolLabelFromRuntime(value),
                        );
      ...
      div({ style: { gridColumnEnd: 'span 6', marginTop: '1.5rem' } }, [
        ...
        div(
          { style: { ...gridStyle, gridGap: '0.7rem', gridTemplateColumns: '4.5rem 9.5rem', marginTop: '0.75rem' } },
          [
            h(NumberInput, {
              min: 1,
              max: 999,
              ...
              tooltip: !isAutopauseEnabled(computeConfig.autopauseThreshold)
                ...
                (id) =>
                  h(Fragment, [
                    ...
                    h(NumberInput, {
                      ...
                      tooltip: !canUpdateNumberOfWorkers
                        ? 'Cloud Compute must be in Running status to change number of workers.'
                        : undefined,
                ...
                (id) =>
                  h(Fragment, [
                    ...
                    h(NumberInput, {
                      ...
                      disabled: !canUpdateNumberOfWorkers,
                      tooltip: !canUpdateNumberOfWorkers
                        ? 'Cloud Compute must be in Running status to change number of preemptibles'
                        : undefined,
          ...
          [
            ...
            },
            {
              ...
              cost: isPersistentDisk
                ? Utils.formatUSD(getPersistentDiskCostMonthly(getPendingDisk(), computeConfig.computeRegion))
                : 'N/A',
  ...
  const renderCustomImageWarning = () => {
    return div({ style: { ...computeStyles.drawerContent, ...computeStyles.warningView } }, [
      ...
      div({ style: { lineHeight: 1.5 } }, [
        ...
        h(Link, { href: safeImageDocumentation, ...Utils.newTabLinkProps }, [
          'Learn more about creating safe and secure custom Docker images.',
        ]),
        p([
          "If you're confident that your image is safe, you may continue using it. Otherwise, go back to select another image.",
        ]),
      ]),
  ...
  const renderDebugger = () => {
    ...
    const makeJSON = (value) =>
      div({ style: { whiteSpace: 'pre-wrap', fontFamily: 'Menlo, monospace' } }, [JSON.stringify(value, null, 2)]);
    return showDebugger
      ? div(
          {
            style: {
      ...
      h(
        ...
        [
          p([
            ...
            code({ style: { fontWeight: 600 } }, [getCurrentMountDirectory(currentRuntimeDetails)]),
            ' to ensure it’s stored on your disk.',
          ]),
          p([
      ...
      options: [
        {
          ...
          options: getImages(
            ({ isCommunity, id }) => !isCommunity && !(getToolLabelForImage(id) === toolLabels.RStudio),
          ),
        },
        {
      ...
      div({ style: { padding: '1.5rem', overflowY: 'auto', flex: 'auto' } }, [
        ...
        !!isPersistentDisk &&
          h(PersistentDiskSection, {
            diskExists: !!existingPersistentDisk,
            computeConfig,
            updateComputeConfig,
        ...
        isGce(runtimeType) &&
          ...
          div({ style: { ...computeStyles.whiteBoxContainer, marginTop: '1rem' } }, [
            div([
              ...
              h(Link, { onClick: handleLearnMoreAboutPersistentDisk }, [
