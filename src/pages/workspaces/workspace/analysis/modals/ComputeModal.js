...
import { b, br, code, div, fieldset, h, label, legend, p, span, strong } from 
'react-hyperscript-helpers'
import { ClipboardButton } from 'src/components/ClipboardButton'
import { ButtonOutline, ButtonPrimary, GroupedSelect, IdContainer, LabeledCheckbox, 
Link, Select, spinnerOverlay } from 'src/components/common'
import { icon } from 'src/components/icons'
import { ImageDepViewer } from 'src/components/ImageDepViewer'
import { NumberInput, TextInput, ValidatedInput } from 'src/components/input'
import { withModalDrawer } from 'src/components/ModalDrawer'
import { InfoBox } from 'src/components/PopupTrigger'
import { getAvailableComputeRegions, getLocationType, getRegionInfo, 
isLocationMultiRegion, isUSLocation } from 'src/components/region-common'
import TitleBar from 'src/components/TitleBar'
import TooltipTrigger from 'src/components/TooltipTrigger'
import { cloudServices, isMachineTypeSmaller, machineTypes } from 
'src/data/gce-machines'
...
import { WarningTitle } from 
'src/pages/workspaces/workspace/analysis/modals/WarningTitle'
import { RadioBlock, SaveFilesHelp, SaveFilesHelpRStudio } from 
'src/pages/workspaces/workspace/analysis/runtime-common-components'
import { getPersistentDiskCostMonthly, runtimeConfigBaseCost, runtimeConfigCost } from 
...
defaultDataprocMachineType, defaultGpuType, defaultLocation,
  defaultNumDataprocPreemptibleWorkers, defaultNumDataprocWorkers, defaultNumGpus, 
displayNameForGpuType, findMachineType, getAutopauseThreshold,
...
const anVILRStudioImage = 
'https://github.com/anvilproject/anvil-docker/tree/master/anvil-rstudio-bioconductor'
const safeImageDocumentation = 
'https://support.terra.bio/hc/en-us/articles/360034669811'

// Distilled from 
https://github.com/docker/distribution/blob/95daa793b83a21656fe6c13e6d5cf1c3999108c7/reference/regexp.go
const imageValidationRegexp = 
/^[A-Za-z0-9]+[\w./-]+(?::\w[\w.-]+)?(?:@[\w+.-]+:[A-Fa-f0-9]{32,})?$/
 ...
 * 2. A cluster with a main node AND two or more worker nodes (sometimes referred to as 
'Spark cluster')
 * If you modify this object, make sure to update the helpers below it too as 
applicable.
 */
const runtimeTypes = {
  gceVm: 'Standard VM',
...
const sparkInterfaces = {
  yarn: {
    ...
    synopsis: 'YARN Resource Manager provides information about cluster status and 
metrics as well as information about the scheduler, nodes, and applications on the 
cluster.'
  },
  appHistory: {
    label: 'apphistory',
    displayName: 'YARN Application Timeline',
    synopsis: 'YARN Application Timeline provides information about current and 
historic applications executed on the cluster.'
  },
  sparkHistory: {
    label: 'sparkhistory',
    displayName: 'Spark History Server',
    synopsis: 'Spark History Server provides information about completed Spark 
applications on the cluster.'
  },
  jobHistory: {
    label: 'jobhistory',
    displayName: 'MapReduce History Server',
    synopsis: 'MapReduce History Server displays information about completed MapReduce 
      ...
      id => h(Fragment, [
        ...
        div([
          h(Select, {
            ...
            isSearchable: false,
            value: currentCpu,
            onChange: option => onChange(_.find({ cpu: option.value }, 
machineTypeOptions)?.name || value),
            options: _.flow(_.map('cpu'), _.union([currentCpu]), 
_.sortBy(_.identity))(machineTypeOptions)
          })
        ])
      ])
    ]),
      ...
      id => h(Fragment, [
        ...
        div([
          h(Select, {
            ...
            isSearchable: false,
            value: currentMemory,
            onChange: option => onChange(_.find({ cpu: currentCpu, memory: option.value 
}, machineTypeOptions)?.name || value),
            options: _.flow(_.filter({ cpu: currentCpu }), _.map('memory'), 
_.union([currentMemory]), _.sortBy(_.identity))(machineTypeOptions)
          })
        ])
      ])
    ])
...
const SparkInterface = ({ sparkInterface, namespace, name, onDismiss }) => {
  ...
  return div(
    { style: { ...computeStyles.whiteBoxContainer, marginBottom: '1rem', 
backgroundColor: colors.accent(0.1), boxShadow: Style.standardShadow } }, [
      div({ style: { flex: '1', lineHeight: '1.5rem', minWidth: 0, display: 'flex' } }, 
[
        div([
          div({ style: { ...computeStyles.headerText, marginTop: '0.5rem' } }, 
[displayName]),
          p([synopsis]),
          div({ style: { display: 'flex', marginTop: '1rem' } }, [
            h(ButtonOutline, {
              href: Nav.getLink('workspace-spark-interface-launch', { namespace, name, 
application: 'spark', sparkInterface: label }),
...
const isDataproc = runtimeType => runtimeType === runtimeTypes.dataprocSingleNode || 
runtimeType === runtimeTypes.dataprocCluster

const isDataprocCluster = runtimeType => runtimeType === runtimeTypes.dataprocCluster

const getImageUrl = runtimeDetails => {
  return _.find(({ imageType }) => _.includes(imageType, ['Jupyter', 'RStudio']), 
runtimeDetails?.runtimeImages)?.imageUrl
}
...
export const ComputeModalBase = ({
  onDismiss, onError, onSuccess, currentRuntime, currentDisk, tool, workspace, 
location, shouldHideCloseButton = true
}) => {
  // State -- begin
  const [showDebugger, setShowDebugger] = useState(false)
  const [loading, setLoading] = useState(false)
  const [currentRuntimeDetails, setCurrentRuntimeDetails] = useState(currentRuntime)
  const [currentPersistentDiskDetails, setCurrentPersistentDiskDetails] = 
useState(currentDisk)
  ...
  const [computeConfig, setComputeConfig] = useState({
    ...
    componentGatewayEnabled: true, // We enable web interfaces (aka Spark console) for 
all new Dataproc clusters.
    gpuEnabled: false,
    hasGpu: false,
  ...
  const isPersistentDisk = shouldUsePersistentDisk(runtimeType, currentRuntimeDetails, 
upgradeDiskSelected)
  ...
  const { version, updated, packages, requiresSpark } = _.find({ image: 
selectedLeoImage }, leoImages) || {}
  // The memory sizes below are the minimum required to launch Terra-supported GCP 
runtimes, based on experimentation.
  const minRequiredMemory = isDataproc(runtimeType) ? 7.5 : 3.75 // in GB
  const validMachineTypes = _.filter(({ memory }) => memory >= minRequiredMemory, 
machineTypes)
  const mainMachineType = _.find({ name: computeConfig.masterMachineType }, 
validMachineTypes)?.name || getDefaultMachineType(isDataproc(runtimeType), tool)
  const machineTypeConstraints = { inclusion: { within: _.map('name', 
validMachineTypes), message: 'is not supported' } }

  const isRuntimeRunning = currentRuntimeDetails?.status === 'Running'
  const shouldDisplaySparkConsoleLink = isDataproc(runtimeType) && 
currentRuntimeDetails?.runtimeConfig?.componentGatewayEnabled
  ...
  const errors = validate(
    { mainMachineType, workerMachineType: computeConfig.workerMachineType, 
customEnvImage },
    {
...
{}
    ...
    {
      prettify: v => ({ customEnvImage: 'Container image', masterMachineType: 'Main 
CPU/memory', workerMachineType: 'Worker CPU/memory' }[v] ||
        validate.prettify(v))
    }
  )
  ...
  const applyChanges = _.flow(
    ...
    const { runtime: existingRuntime, persistentDisk: existingPersistentDisk } = 
getExistingEnvironmentConfig()
    const { runtime: desiredRuntime, persistentDisk: desiredPersistentDisk } = 
getDesiredEnvironmentConfig()
    const shouldUpdatePersistentDisk = canUpdatePersistentDisk() && 
!_.isEqual(desiredPersistentDisk, existingPersistentDisk)
    const shouldDeletePersistentDisk = existingPersistentDisk && 
!canUpdatePersistentDisk()
    const shouldUpdateRuntime = canUpdateRuntime() && !_.isEqual(desiredRuntime, 
existingRuntime)
    ...
    if (shouldDeleteRuntime) {
      await Ajax().Runtimes.runtime(googleProject, 
currentRuntimeDetails.runtimeName).delete(hasAttachedDisk() && 
shouldDeletePersistentDisk)
    }
    if (shouldDeletePersistentDisk && !hasAttachedDisk()) {
      await Ajax().Disks.disk(googleProject, 
currentPersistentDiskDetails.name).delete()
    }
    ...
    if (shouldUpdateRuntime || shouldCreateRuntime) {
      const runtimeConfig = desiredRuntime && {
        ...
        ...(desiredRuntime.cloudService === cloudServices.GCE ? {
          ...
          machineType: desiredRuntime.machineType || getDefaultMachineType(false, 
desiredToolLabel),
          ...(computeConfig.gpuEnabled && { gpuConfig: { gpuType: 
computeConfig.gpuType, numOfGpus: computeConfig.numGpus } })
        } : {
          region: desiredRuntime.region.toLowerCase(),
          masterMachineType: desiredRuntime.masterMachineType || 
defaultDataprocMachineType,
      ...
      if (shouldUpdateRuntime) {
        const updateRuntimeConfig = _.merge(shouldUpdatePersistentDisk ? { diskSize: 
desiredPersistentDisk.size } : {}, runtimeConfig)

        await Ajax().Runtimes.runtime(googleProject, 
currentRuntimeDetails.runtimeName).update({
      ...
      if (shouldCreateRuntime) {
        const diskConfig = Utils.cond(
          ...
          [existingPersistentDisk && (!shouldDeletePersistentDisk || 
shouldUpdatePersistentDisk), () => ({ persistentDisk: { name: 
currentPersistentDiskDetails.name } })],
          [Utils.DEFAULT, () => ({
            persistentDisk: {
        ...
        if (shouldUpdatePersistentDisk) {
          await Ajax().Disks.disk(googleProject, 
currentPersistentDiskDetails.name).update(desiredPersistentDisk.size)
        }

        const createRuntimeConfig = { ...runtimeConfig, ...diskConfig }
        await Ajax().Runtimes.runtime(googleProject, 
Utils.generateRuntimeName()).create({
          ...
          ...(desiredRuntime.jupyterUserScriptUri ? { jupyterUserScriptUri: 
desiredRuntime.jupyterUserScriptUri } : {})
        })
      }
    }
  ...
  const getMainMachineTypeByNumCpus = numCpus => _.find({ cpu: numCpus }, 
validMachineTypes)?.name || mainMachineType

  const getMainMachineTypeByMemory = (numCpus, memory) => _.find({ cpu: numCpus, memory 
}, validMachineTypes)?.name || mainMachineType

  const getValidCpuGpuConfig = machineType => {
    const { cpu: currentNumCpus, memory: currentMemory } = findMachineType(machineType)
    const validGpuOptions = getValidGpuOptions(currentNumCpus, currentMemory, 
computeConfig.computeZone)
    const validGpuNames = _.flow(_.map('name'), _.uniq, 
_.sortBy('price'))(validGpuOptions)
    const validGpuName = _.includes(displayNameForGpuType(computeConfig.gpuType), 
validGpuNames) ?
    ...
    const validNumGpusOptions = _.flow(_.filter({ name: validGpuName }), 
_.map('numGpus'))(validGpuOptions)
    const validNumGpus = _.includes(computeConfig.numGpus, validNumGpusOptions) ? 
computeConfig.numGpus : _.head(validNumGpusOptions)

    return { currentNumCpus, currentMemory, validGpuName, validGpuNames, validGpuType, 
validGpuOptions, validNumGpus, validNumGpusOptions }
  }

  const isRStudioImage = getToolLabelForImage(_.find({ image: selectedLeoImage }, 
leoImages)?.id) === runtimeTools.RStudio.label

  const canUpdateRuntime = () => {
    const { runtime: existingRuntime, autopauseThreshold: existingAutopauseThreshold } 
= getExistingEnvironmentConfig()
    const { runtime: desiredRuntime, autopauseThreshold: desiredAutopauseThreshold } = 
getDesiredEnvironmentConfig()
    ...
    return !(
      ...
      (desiredRuntime.cloudService === cloudServices.GCE ? (
        desiredRuntime.persistentDiskAttached !== 
existingRuntime.persistentDiskAttached ||
        desiredAutopauseThreshold !== existingAutopauseThreshold ||
        (desiredRuntime.persistentDiskAttached ? !canUpdatePersistentDisk() : 
desiredRuntime.diskSize < existingRuntime.diskSize)
      ) : (
...
||
  ...
  const getCurrentMountDirectory = currentRuntimeDetails => {
    ...
    const noMountDirectory = `${jupyterMountPoint} for Jupyter environments and 
${rstudioMountPoint} for RStudio environments`
    return currentRuntimeDetails?.labels.tool ?
      (currentRuntimeDetails?.labels.tool === 'RStudio' ? rstudioMountPoint : 
jupyterMountPoint) :
      noMountDirectory
  }
  ...
  const getExistingEnvironmentConfig = () => {
    ...
    return {
      ...
      runtime: currentRuntimeDetails ? {
        ...
        tool: toolLabel,
        ...(currentRuntimeDetails?.jupyterUserScriptUri && { jupyterUserScriptUri: 
currentRuntimeDetails?.jupyterUserScriptUri }),
        ...(cloudService === cloudServices.GCE ? {
          zone: computeConfig.computeZone,
          machineType: runtimeConfig.machineType || getDefaultMachineType(false, 
toolLabel),
...
isDataproc(runtimeType),
          ...(numberOfWorkers && {
            numberOfPreemptibleWorkers: runtimeConfig.numberOfPreemptibleWorkers || 0,
            workerMachineType: runtimeConfig.workerMachineType || 
defaultDataprocMachineType,
            workerDiskSize: runtimeConfig.workerDiskSize || 100
          })
        })
      } : undefined,
      persistentDisk: currentPersistentDiskDetails ? { size: 
currentPersistentDiskDetails.size, diskType: currentPersistentDiskDetails.diskType } : 
undefined
    }
  }

  const getDesiredEnvironmentConfig = () => {
    const { persistentDisk: existingPersistentDisk, runtime: existingRuntime } = 
getExistingEnvironmentConfig()
    const cloudService = isDataproc(runtimeType) ? cloudServices.DATAPROC : 
cloudServices.GCE
    const desiredNumberOfWorkers = isDataprocCluster(runtimeType) ? 
computeConfig.numberOfWorkers : 0
    ...
    return {
      ...
      runtime: Utils.cond(
        [(viewMode !== 'deleteEnvironment'), () => {
          return {
            cloudService,
            toolDockerImage: selectedLeoImage === customMode ? customEnvImage : 
selectedLeoImage,
            ...(jupyterUserScriptUri && { jupyterUserScriptUri }),
            ...(cloudService === cloudServices.GCE ? {
              zone: computeConfig.computeZone,
              region: computeConfig.computeRegion,
              machineType: computeConfig.masterMachineType || 
getDefaultMachineType(false, getToolLabelFromRuntime(existingRuntime)),
              ...(computeConfig.gpuEnabled ? { gpuConfig: { gpuType: 
computeConfig.gpuType, numOfGpus: computeConfig.numGpus } } : {}),
              bootDiskSize: existingRuntime?.bootDiskSize,
              ...(shouldUsePersistentDisk(runtimeType, currentRuntimeDetails, 
upgradeDiskSelected) ? {
                persistentDiskAttached: true
              } : {
              ...
              region: computeConfig.computeRegion,
              masterMachineType: computeConfig.masterMachineType || 
defaultDataprocMachineType,
              ...
              ...(desiredNumberOfWorkers && {
                ...
                workerMachineType: computeConfig.workerMachineType || 
defaultDataprocMachineType,
        ...
        [!deleteDiskSelected || existingRuntime?.persistentDiskAttached, () => 
undefined],
        () => existingRuntime
      ),
      persistentDisk: Utils.cond(
        [deleteDiskSelected, () => undefined],
        [viewMode !== 'deleteEnvironment' && shouldUsePersistentDisk(runtimeType, 
currentRuntimeDetails, upgradeDiskSelected),
          () => ({ size: computeConfig.selectedPersistentDiskSize, diskType: 
computeConfig.selectedPersistentDiskType })],
        () => existingPersistentDisk
      )
    }
  }
  ...
  const getPendingDisk = () => {
    const { persistentDisk: { size = 0, diskType = pdTypes.standard } = {} } = 
getDesiredEnvironmentConfig()
...
defaultDataprocMachineType,
  ...
  const handleLearnMoreAboutPersistentDisk = () => {
    ...
    Ajax().Metrics.captureEvent(Events.aboutPersistentDiskView, {
      ...extractWorkspaceDetails(getWorkspaceObject()), currentlyHasAttachedDisk: 
!!hasAttachedDisk()
    })
  }
...
'', selectedImage.id)}/CHANGELOG.md`

    return div({ style: { whiteSpace: 'pre', ...style } }, [
      div({ style: Style.proportionalNumbers }, ['Updated: ', updated ? 
Utils.makeStandardDate(updated) : null]),
      h(Link, {
        href: changelogUrl,
...
getExistingEnvironmentConfig()
    const desiredMachineType = desiredRuntime &&
      (desiredRuntime.cloudService === cloudServices.GCE ? desiredRuntime.machineType : 
desiredRuntime.masterMachineType)
    const existingMachineType = existingRuntime &&
      (existingRuntime?.cloudService === cloudServices.GCE ? 
existingRuntime.machineType : existingRuntime.masterMachineType)
    const { cpu: desiredRuntimeCpus, memory: desiredRuntimeMemory } = 
findMachineType(desiredMachineType)
    const { cpu: existingRuntimeCpus, memory: existingRuntimeMemory } = 
findMachineType(existingMachineType)
    ...
    Ajax().Metrics.captureEvent(Events[metricsEvent], {
      ...
      desiredRuntime_costPerHour: desiredRuntime && 
runtimeConfigCost(getPendingRuntimeConfig(), getPendingDisk()),
      desiredRuntime_pausedCostPerHour: desiredRuntime && 
runtimeConfigBaseCost(getPendingRuntimeConfig(), getPendingDisk()),
...
getPersistentDiskCostMonthly(getPendingDisk(), computeConfig.computeRegion)),
  ...
  const updateComputeConfig = _.curry((key, value) => setComputeConfig(_.set(key, 
value)))

  const willDeleteBuiltinDisk = () => {
    const { runtime: existingRuntime } = getExistingEnvironmentConfig()
    return (existingRuntime?.diskSize || existingRuntime?.masterDiskSize) && 
!canUpdateRuntime()
  }
  ...
  const isDifferentLocation = () => {
    if (isLocationMultiRegion(location)) {
      // For a US multi-regional bucket, the computeRegion needs to be US-CENTRAL1 in 
order to be considered "in the same location".
      // Currently, US is the only multi-region supported in Terra
      return computeConfig.computeRegion !== defaultComputeRegion
    } else {
      // If the bucket is regional, we can easily compare the bucketLocation with the 
compute region.
  ...
  const getLocationTooltip = (computeExists, bucketLocation) => Utils.cond(
    [computeExists,
      () => 'Cannot update the location of an existing cloud environment. Delete your 
cloud environment to create a new one in a different region.'],
    [isUSLocation(bucketLocation), () => 'Currently US workspaces can only have US 
cloud environments.'],
    [Utils.DEFAULT, () => 'Cloud environments run in the same region as the workspace 
bucket and can be changed as a beta feature.']
  )
  // Helper functions -- end

  // Lifecycle
  useOnMount(() => {
    // Can't pass an async function into useEffect so we define the function in the 
body and then call it
...
...extractWorkspaceDetails(getWorkspaceObject())
      ...
      const [currentRuntimeDetails, newLeoImages, currentPersistentDiskDetails] = await 
Promise.all([
        currentRuntime ? Ajax().Runtimes.runtime(currentRuntime.googleProject, 
currentRuntime.runtimeName).details() : null,
        Ajax()
          .Buckets
          .getObjectPreview(googleProject, getConfig().terraDockerImageBucket, 
getConfig().terraDockerVersionsFile, true)
          .then(r => r.json()),
        currentDisk ? Ajax().Disks.disk(currentDisk.googleProject, 
currentDisk.name).details() : null
      ])

      const filteredNewLeoImages = !!tool ? _.filter(image => _.includes(image.id, 
runtimeTools[tool].imageIds), newLeoImages) : newLeoImages

      const imageUrl = currentRuntimeDetails ? getImageUrl(currentRuntimeDetails) : 
_.find({ id: 'terra-jupyter-gatk' }, newLeoImages).image
      const foundImage = _.find({ image: imageUrl }, newLeoImages)

      /* eslint-disable indent */
      // Selected Leo image uses the following logic (psuedoCode not written in same 
way as code for clarity)
      // if found image (aka image associated with user's runtime) NOT in newLeoImages 
(the image dropdown list from bucket)
      //   user is using custom image
      // else if found Image NOT in filteredNewLeoImages (filtered based on analysis 
tool selection)
      ...
      const getSelectedImage = () => {
        if (foundImage) {
          if (!_.includes(foundImage, filteredNewLeoImages)) {
            return _.find({ id: runtimeTools[tool].defaultImageId }, 
newLeoImages).image
          } else {
            return imageUrl
          }
      ...
      const { computeZone, computeRegion } = getRegionInfo(location || defaultLocation, 
locationType)
      const runtimeConfig = currentRuntimeDetails?.runtimeConfig || computeConfig
      const gpuConfig = runtimeConfig?.gpuConfig
      const autopauseThresholdCalculated = !!currentRuntimeDetails ? 
currentRuntimeDetails.autopauseThreshold : defaultAutopauseThreshold
      const newRuntimeType = Utils.switchCase(runtimeConfig?.cloudService,
        [cloudServices.DATAPROC, () => runtimeConfig.numberOfWorkers === 0 ? 
runtimeTypes.dataprocSingleNode : runtimeTypes.dataprocCluster],
        [cloudServices.GCE, () => runtimeTypes.gceVm],
        [Utils.DEFAULT, () => runtimeTypes.gceVm] // for when there's no existing 
runtime
      )

      const diskSize = Utils.cond([!!runtimeConfig?.diskSize, () => 
runtimeConfig.diskSize],
      ...
      setComputeConfig({
        selectedPersistentDiskSize: currentPersistentDiskDetails?.size || 
defaultGcePersistentDiskSize,
        selectedPersistentDiskType: (!!currentPersistentDiskDetails?.diskType && 
currentPersistentDiskDetails.diskType) || defaultPersistentDiskType,
        masterMachineType: runtimeConfig?.masterMachineType || 
runtimeConfig?.machineType,
        masterDiskSize: diskSize,
        numberOfWorkers: runtimeConfig?.numberOfWorkers || 2,
        componentGatewayEnabled: runtimeConfig?.componentGatewayEnabled || 
isDataprocCluster(newRuntimeType),
        numberOfPreemptibleWorkers: runtimeConfig?.numberOfPreemptibleWorkers || 0,
        workerMachineType: runtimeConfig?.workerMachineType || 
defaultDataprocMachineType,
...
[getCurrentMountDirectory(currentRuntimeDetails)]), br(),
          'Please save your analysis data in this directory to ensure it’s stored on 
your disk.']),
        p(['Terra attaches a persistent disk (PD) to your cloud compute in order to 
provide an option to keep the data on the disk after you delete your compute. PDs also 
act as a safeguard to protect your data in the case that something goes wrong with the 
compute.']),
        p(['A minimal cost per hour is associated with maintaining the disk even when 
the cloud compute is paused or deleted.']),
        p(['If you delete your cloud compute, but keep your PD, the PD will be 
reattached when creating the next cloud compute.']),
        h(Link, { href: 'https://support.terra.bio/hc/en-us/articles/360047318551', 
...Utils.newTabLinkProps }, [
  ...
  const renderActionButton = () => {
    ...
    const commonButtonProps = Utils.cond([
        ...
        () => ({ disabled: true, tooltip: 'Cloud compute with GPU(s) cannot be updated. 
Please delete it and create a new one.' })
      ], [
        computeConfig.gpuEnabled && 
_.isEmpty(getValidGpuTypesForZone(computeConfig.computeZone)) && viewMode !== 
...
viewMode !==
        'deleteEnvironment',
        () => ({ disabled: true, tooltip: 'Cannot create environment in location 
differing from existing persistent disk location.' })
      ],
      () => ({ disabled: !hasChanges() || !!errors, tooltip: 
Utils.summarizeErrors(errors) })
    )

    const isRuntimeError = existingRuntime?.status === 'Error'
    const shouldErrorDisableUpdate = existingRuntime?.toolDockerImage === 
desiredRuntime?.toolDockerImage
    const isUpdateDisabled = getIsRuntimeBusy(currentRuntimeDetails) || 
(shouldErrorDisableUpdate && isRuntimeError)

    const canShowWarning = viewMode === undefined
    const canShowEnvironmentWarning = _.includes(viewMode, [undefined, 
'customImageWarning'])

    return Utils.cond([
        canShowWarning && isCustomImage && existingRuntime?.toolDockerImage !== 
desiredRuntime?.toolDockerImage,
        () => h(ButtonPrimary, { ...commonButtonProps, onClick: () => 
setViewMode('customImageWarning') }, ['Next'])
      ], [
        canShowEnvironmentWarning && (willDeleteBuiltinDisk() || 
willDeletePersistentDisk() || willRequireDowntime() || willDetachPersistentDisk()),
        () => h(ButtonPrimary, { ...commonButtonProps, onClick: () => 
setViewMode('environmentWarning') }, ['Next'])
      ], [
        canShowWarning && isDifferentLocation(),
        () => h(ButtonPrimary, { ...commonButtonProps, onClick: () => 
setViewMode('differentLocationWarning') }, ['Next'])
      ], [
        canShowWarning && !isUSLocation(computeConfig.computeRegion),
        () => h(ButtonPrimary, { ...commonButtonProps, onClick: () => 
setViewMode('nonUSLocationWarning') }, ['Next'])
      ],
      () => h(ButtonPrimary, {
        ...commonButtonProps,
        onClick: () => {
        ...
        tooltipSide: 'left',
        tooltip: isUpdateDisabled ? `Cannot perform change on environment in 
(${currentRuntimeDetails.status}) status` : 'Update Environment'
      }, [
        Utils.cond(
    ...
    return h(IdContainer, [
      id => h(div, {}, [
        div({ style: { margin: '0.5rem 0' } }, [
          label({ htmlFor: id, style: { ...computeStyles.label } }, ['Creation Timeout 
Limit']),
          h(InfoBox, { style: { marginLeft: '0.5rem' } }, [
            'Custom and Legacy image creation may take longer than the default 10 
minute timeout window. ' +
            'To avoid an error, you may enter a value between 10 and 30 minutes.'
          ])
        ]),
        div({ style: { display: 'grid', alignItems: 'center', gridGap: '0.7rem', 
gridTemplateColumns: '4.5rem 9.5rem', marginTop: '0.75rem' } }, [
          h(NumberInput, {
              id,
              min: 10,
              max: 30,
      ...
      h(IdContainer, [
        id => h(Fragment, [
          div({ style: { marginBottom: '0.5rem' } }, [
            label({ htmlFor: id, style: computeStyles.label }, ['Application 
configuration']),
            h(InfoBox, { style: { marginLeft: '0.5rem' } }, [
              'The software application + programming languages + packages used when 
      ...
      Utils.switchCase(selectedLeoImage,
        [customMode, () => {
          return h(Fragment, [
            h(IdContainer, [
              id => h(Fragment, [
                label({ htmlFor: id, style: { ...computeStyles.label, display: 'block', 
margin: '0.5rem 0' } }, ['Container image']),
                div({ style: { height: 52 } }, [
                  h(ValidatedInput, {
                    inputProps: {
                      id,
                    ...
                    error: Utils.summarizeErrors(customEnvImage && 
errors?.customEnvImage)
                  })
                ])
              ])
            ]),
        ...
        [Utils.DEFAULT, () => {
          return h(Fragment, [
            div({ style: { display: 'flex' } }, [
              h(Link, { onClick: () => setViewMode('packages') }, ['What’s installed on 
this environment?']),
...
Optional'])]),
      ...
      selectedLeoImage && !supportedImages.includes(selectedLeoImage) ? 
renderCustomTimeoutInMinutes() : []
    ])
  }

  const renderComputeProfileSection = computeExists => {
    const { currentNumCpus, currentMemory, validGpuName, validGpuNames, 
validGpuOptions, validNumGpus, validNumGpusOptions } = getValidCpuGpuConfig(
      mainMachineType)

    const gpuCheckboxDisabled = computeExists ? !computeConfig.gpuEnabled : 
isDataproc(runtimeType) || isRStudioImage
    ...
    const gridStyle = { display: 'grid', gridGap: '1rem', alignItems: 'center', 
marginTop: '1rem' }
    const gridItemInputStyle = { minWidth: '6rem' }

    return div({ style: { ...computeStyles.whiteBoxContainer, marginTop: '1rem' } }, [
      div({ style: { fontSize: '0.875rem', fontWeight: 600 } }, ['Cloud compute 
profile']),
      div([
        div({ style: { ...gridStyle, gridGap: '.75rem', gridTemplateColumns: 'repeat(6, 
auto)', justifyContent: 'flex-start' } }, [
              ...
              div([
                h(Select, {
                  ...
                  onChange: ({ value }) => {
                    ...
                    const { validGpuType: newGpuType, validNumGpus: newNumGpus } = 
getValidCpuGpuConfig(mainMachineType)
                  ...
                  options: _.flow(_.map('cpu'), _.union([currentNumCpus]), 
_.sortBy(_.identity))(validMachineTypes)
                })
              ])
            ])
          ]),
            ...
            id => h(Fragment, [
              ...
              div([
                h(Select, {
                  ...
                  value: currentMemory,
                  onChange: ({ value }) => {
                    const mainMachineType = getMainMachineTypeByMemory(currentNumCpus, 
value)
                    const { validGpuType: newGpuType, validNumGpus: newNumGpus } = 
getValidCpuGpuConfig(mainMachineType)
                    updateComputeConfig('masterMachineType', mainMachineType)
                  ...
                  options: _.flow(_.filter({ cpu: currentNumCpus }), _.map('memory'), 
_.union([currentMemory]), _.sortBy(_.identity))(
                    validMachineTypes)
                })
              ])
            ])
          ...
          // Disk Selection
          !isPersistentDisk ?
            h(DataprocDiskSelector, { value: computeConfig.masterDiskSize, onChange: 
updateComputeConfig('masterDiskSize') }) :
            div({ style: { gridColumnEnd: 'span 2' } })
        ]),
        // GPU Enabling
        isGce(runtimeType) && div({ style: { gridColumnEnd: 'span 6', marginTop: 
'1.5rem' } }, [
          h(LabeledCheckbox, {
            ...
            span({ style: { marginLeft: '0.5rem', ...computeStyles.label, 
verticalAlign: 'top' } }, [
              gpuCheckboxDisabled ?
                h(TooltipTrigger, {
                  content: isRStudioImage ?
                    'GPUs are not currently supported for the selected application 
configuration.' :
...
...Utils.newTabLinkProps
        ...
        div({ style: { ...gridStyle, gridTemplateColumns: '0.75fr 12rem 1fr 5.5rem 1fr 
5.5rem' } }, [
          h(Fragment, [
            h(IdContainer, [
              id => h(Fragment, [
                ...
                div([
                  h(Select, {
                    ...
                    onChange: ({ value }) => updateComputeConfig('numGpus',
                      _.get('numGpus', _.find({ type: computeConfig.gpuType, numGpus: 
value }, validGpuOptions))),
        ...
        div({ style: gridStyle }, [
          h(IdContainer, [
            id => div({ style: { gridColumnEnd: 'span 4', marginTop: '0.5rem' } }, [
              ...
              (isRStudioImage || requiresSpark) && h(InfoBox, { style: { marginLeft: 
'0.5rem' } }, [
                'Only the compute types compatible with the selected application 
configuration are made available below.'
              ]),
                ...
                div({ style: { flex: 1, marginRight: '2rem' } }, [
                  h(Select, {
                    ...
                    onChange: ({ value }) => {
                      ...
                      const defaultMachineTypeForSelectedValue = 
getDefaultMachineType(isDataproc(value), getToolLabelFromRuntime(value))
                      // we need to update the compute config if the current value is 
smaller than the default for the dropdown option
                      if (isMachineTypeSmaller(computeConfig.masterMachineType, 
defaultMachineTypeForSelectedValue)) {
                        updateComputeConfig('masterMachineType', 
defaultMachineTypeForSelectedValue)
                      }
                    ...
                    options: [
                      ...
                      { value: runtimeTypes.dataprocSingleNode, isDisabled: 
isRStudioImage },
                ...
                shouldDisplaySparkConsoleLink && span([
                  h(Link, {
                    ...
                    tooltip: !canManageSparkConsole && 'You must have a running Spark 
cluster or a master node.',
      ...
      div({ style: { gridColumnEnd: 'span 6', marginTop: '1.5rem' } }, [
        h(LabeledCheckbox, {
          ...
          onChange: v => updateComputeConfig('autopauseThreshold', 
getAutopauseThreshold(v))
        }, [
          span({ style: { marginLeft: '0.5rem', ...computeStyles.label, verticalAlign: 
'top' } }, [
            enableAutopauseSpan
          ]),
        ]),
        h(Link, {
          style: { marginLeft: '1rem', verticalAlign: 'top' },
          href: 
'https://support.terra.bio/hc/en-us/articles/360029761352-Preventing-runaway-costs-with-Cloud-Environment-autopause-#h_27c11f46-a6a7-4860-b5e7-fac17df2b2b5', 
...Utils.newTabLinkProps
        }, [
          'Learn more about autopause.',
          icon('pop-out', { size: 12, style: { marginLeft: '0.25rem' } })
        ]),
        div({ style: { ...gridStyle, gridGap: '0.7rem', gridTemplateColumns: '4.5rem 
9.5rem', marginTop: '0.75rem' } }, [
          h(NumberInput, {
            min: 1,
            max: 999,
            isClearable: false,
            ...
            tooltip: !isAutopauseEnabled(computeConfig.autopauseThreshold) ? 'Autopause 
must be enabled to configure pause time.' : undefined,
            onChange: updateComputeConfig('autopauseThreshold'),
            'aria-label': 'Minutes of inactivity before autopausing'
          }),
          span({ hidden: !isAutopauseEnabled(computeConfig.autopauseThreshold) }, 
['minutes of inactivity'])
        ])
      ]),
      isDataprocCluster(runtimeType) && fieldset({ style: { margin: '1.5rem 0 0', 
border: 'none', padding: 0 } }, [
        legend({ style: { padding: 0, ...computeStyles.label } }, ['Worker config']),
        // grid styling in a div because of display issues in chrome: 
https://bugs.chromium.org/p/chromium/issues/detail?id=375693
        div({ style: { ...gridStyle, gridGap: '.75rem', gridTemplateColumns: '0.25fr 
5rem 1fr 5.5rem 1fr 5.5rem', marginTop: '0.75rem' } }, [
          h(IdContainer, [
            id => h(Fragment, [
              ...
              h(NumberInput, {
                ...
                tooltip: !canUpdateNumberOfWorkers ? 'Cloud Compute must be in Running 
status to change number of workers.' : undefined,
          ...
          h(IdContainer, [
            id => h(Fragment, [
              ...
              h(NumberInput, {
                ...
                disabled: !canUpdateNumberOfWorkers,
                tooltip: !canUpdateNumberOfWorkers ? 'Cloud Compute must be in Running 
status to change number of preemptibles' : undefined,
          ...
          div({ style: { gridColumnEnd: 'span 2' } }),
          h(WorkerSelector, {
            value: computeConfig.workerMachineType, machineTypeOptions: 
validMachineTypes, onChange: updateComputeConfig('workerMachineType')
          }),
          h(DataprocDiskSelector, { value: computeConfig.workerDiskSize, onChange: 
updateComputeConfig('workerDiskSize') })
        ])
      ]),
      div({ style: { ...gridStyle, gridTemplateColumns: '0.25fr 8.5rem 1fr 5.5rem 1fr 
5rem', marginTop: '1.5rem' } }, [
        h(IdContainer, [
          id => div({ style: { gridColumnEnd: 'span 3' } }, [
            ...
            div({ style: { marginTop: '0.5rem' } }, [
              h(Select, {
                ...
                // 1) If editing an existing environment (can't update location of 
existing environments)
                // 2) Workspace buckets that are either us-central1 or us 
multi-regional
                isDisabled: computeExists || isUSLocation(location),
                isSearchable: false,
                value: computeConfig.computeRegion,
                onChange: ({ value, locationType }) => updateComputeLocation(value, 
locationType),
  ...
  // TODO [IA-3348] parameterize and make it a shared function between the equivalent 
in AzureComputeModal
  const renderCostBreakdown = () => {
    return div({
      ...
      _.map(({ cost, label, unitLabel }) => {
        ...
        { label: 'Running cloud compute cost', cost: 
Utils.formatUSD(runtimeConfigCost(getPendingRuntimeConfig(), getPendingDisk())), 
unitLabel: 'per hr' },
        { label: 'Paused cloud compute cost', cost: 
Utils.formatUSD(runtimeConfigBaseCost(getPendingRuntimeConfig(), getPendingDisk())), 
unitLabel: 'per hr' },
        {
          label: 'Persistent disk cost',
          cost: isPersistentDisk ? 
Utils.formatUSD(getPersistentDiskCostMonthly(getPendingDisk(), 
computeConfig.computeRegion)) : 'N/A',
...
terraBaseImages, ...Utils.newTabLinkProps }, ['Terra base images.'])]
        ...
        h(Link, { href: safeImageDocumentation, ...Utils.newTabLinkProps }, ['Learn 
more about creating safe and secure custom Docker images.']),
        p(['If you\'re confident that your image is safe, you may continue using it. 
Otherwise, go back to select another image.'])
      ]),
  ...
  const renderDifferentLocationWarning = () => {
    return div({ style: { ...computeStyles.drawerContent, ...computeStyles.warningView 
      ...
      h(TitleBar, {
        ...
        hideCloseButton: shouldHideCloseButton,
        style: computeStyles.titleBar,
        title: h(WarningTitle, ['Compute location differs from workspace bucket 
location']),
        onDismiss,
        onPrevious: () => setViewMode(undefined)
      }),
      div({ style: { lineHeight: 1.5 } }, [
        p([
          ...
          ' may incur network egress charges. Note that network egress charges are not 
accounted for in cost estimates.'
        ]),
        h(Link, { href: 'https://support.terra.bio/hc/en-us/articles/360058964552', 
...Utils.newTabLinkProps }, [
  ...
  const renderNonUSLocationWarning = () => {
    return div({ style: { ...computeStyles.drawerContent, ...computeStyles.warningView 
      ...
      div({ style: { lineHeight: 1.5 } }, [
        p(['Having a Cloud Environment outside of the US is currently a beta 
feature.']),
        h(Link, { href: 'https://support.terra.bio/hc/en-us/articles/360058964552', 
...Utils.newTabLinkProps }, [
          'For more information please read the documentation.',
  ...
  const renderDebugger = () => {
    const makeHeader = text => div({ style: { fontSize: 20, margin: '0.5rem 0' } }, 
[text])
    const makeJSON = value => div({ style: { whiteSpace: 'pre-wrap', fontFamily: 
'Menlo, monospace' } }, [JSON.stringify(value, null, 2)])
    return showDebugger ?
      div({ style: { position: 'fixed', top: 0, left: 0, bottom: 0, right: '50vw', 
backgroundColor: 'white', padding: '1rem', overflowY: 'auto' } }, [
        h(Link, { 'aria-label': 'Hide debugger', onClick: () => setShowDebugger(false), 
style: { position: 'absolute', top: 0, right: 0 } }, ['x']),
      ...
      h(Link, { 'aria-label': 'Show debugger', onClick: () => setShowDebugger(true), 
  ...
  const renderDeleteDiskChoices = () => {
    ...
    return h(Fragment, [
      h(RadioBlock, {
        ...
        labelText: 'Keep persistent disk, delete application configuration and compute 
profile',
        ...
        p(['Please save your analysis data in the directory ',
          code({ style: { fontWeight: 600 } }, 
[getCurrentMountDirectory(currentRuntimeDetails)]), ' to ensure it’s stored on your 
disk.']),
        p([
          'Deletes your application configuration and cloud compute profile, but 
detaches your persistent disk and saves it for later. ',
          'The disk will be automatically reattached the next time you create a cloud 
environment using the standard VM compute type.'
        ]),
        p({ style: { marginBottom: 0 } }, [
          'You will continue to incur persistent disk cost at ',
          span({ style: { fontWeight: 600 } },
            [Utils.formatUSD(getPersistentDiskCostMonthly(currentPersistentDiskDetails, 
computeConfig.computeRegion)), ' per month.'])
        ])
      ]),
      h(RadioBlock, {
        ...
        p([
          'Deletes your persistent disk, which will also ', span({ style: { fontWeight: 
600 } }, ['delete all files on the disk.'])
        ]),
        p({ style: { marginBottom: 0 } }, [
...
profile',
                ...
                p({ style: { marginBottom: 0 } }, [
                  'Deletes your application configuration and cloud compute profile. 
This will also ',
                  span({ style: { fontWeight: 600 } }, ['delete all files on the 
built-in hard disk.'])
                ])
              ]),
              h(RadioBlock, {
                ...
                p([
                  'Deletes your persistent disk, which will also ', span({ style: { 
fontWeight: 600 } }, ['delete all files on the disk.'])
                ]),
                p({ style: { marginBottom: 0 } }, [
...
configuration and cloud compute profile will remain.'
              ...
              existingRuntime.tool === 'RStudio' ? h(SaveFilesHelpRStudio) : 
h(SaveFilesHelp)
            ])
          }],
...
deleting it, you will need to create a new cloud environment to access it.'
                ])
              ]),
              // At this point there is no runtime (we're in the !existingRuntime 
block) to check the tool
          ...
          () => {
            return h(Fragment, [
              p([
                'Deleting your application configuration and cloud compute profile will 
also ',
                span({ style: { fontWeight: 600 } }, ['delete all files on the built-in 
hard disk.'])
              ]),
              existingRuntime.tool === 'RStudio' ? h(SaveFilesHelpRStudio) : 
h(SaveFilesHelp)
  ...
  const renderEnvironmentWarning = () => {
    const { runtime: existingRuntime } = getExistingEnvironmentConfig()
    const desiredToolLabel = getToolLabelForImage(_.find({ image: selectedLeoImage }, 
leoImages)?.id)
    const desiredToolLabelPhrase = isCustomImage ? 'a custom image' : 
strong([desiredToolLabel])
    ...
    return div({ style: { ...computeStyles.drawerContent, ...computeStyles.warningView 
      ...
      h(TitleBar, {
        ...
        title: h(WarningTitle, [
          Utils.cond(
            [willDetachPersistentDisk(), () => 'Replace application configuration and 
cloud compute profile for Spark'],
            [willDeleteBuiltinDisk() || willDeletePersistentDisk(), () => 'Data will be 
deleted'],
      ...
      div({ style: { lineHeight: 1.5 } }, [
        Utils.cond(
          [willDetachPersistentDisk(), () => h(Fragment, [
            div([
              'You have requested to replace your existing application configuration 
and cloud compute profile to ones that support Spark. ',
              'This type of cloud compute does not support the persistent disk 
feature.'
            ]),
            div({ style: { margin: '1rem 0 0.5rem', fontSize: 16, fontWeight: 600 } }, 
['What would you like to do with your disk?']),
          ...
          [willDeleteBuiltinDisk(), () => h(Fragment, [
            p([
              ...
              span({ style: { fontWeight: 600 } }, ['delete all files on built-in hard 
disk.'])
            ]),
            existingRuntime?.tool === 'RStudio' ? h(SaveFilesHelpRStudio) : 
h(SaveFilesHelp)
          ...
          [willDeletePersistentDisk(), () => h(Fragment, [
            p([
              'To reduce the size of the PD, the existing PD will be deleted and a new 
one will be created and attached to your virtual machine instance. This will ',
...
h(SaveFilesHelp)
          ...
          [willRequireDowntime(), () => h(Fragment, [
            existingRuntime && existingRuntime.tool !== desiredToolLabel ?
              p([
                'By continuing, you will be changing the application of your cloud 
environment from ',
              ...
              undefined,
            p(['This change will require temporarily shutting down your cloud 
environment. You will be unable to perform analysis for a few minutes.']),
    ...
    return h(GroupedSelect, {
      ...
      onChange: ({ value }) => {
        ...
        const newRuntimeType = Utils.cond(
          ...
          [!!requiresSpark && !isDataproc(runtimeType), () => 
runtimeTypes.dataprocSingleNode],
        ...
        const machineType = getDefaultMachineType(isDataproc(newRuntimeType), 
desiredToolLabel)
        updateComputeConfig('masterMachineType', machineType)
        if (isDataproc(newRuntimeType) && computeConfig.masterDiskSize < 
defaultDataprocMasterDiskSize) {
...
getExistingEnvironmentConfig()
    const renderTitleAndTagline = () => {
      return h(Fragment, [
        ...
        div(['A cloud environment consists of application configuration, cloud compute 
and persistent disk(s).']),
      ])
    }
      ...
      return div({ style: { display: 'flex', marginTop: '2rem' } }, [
        (!!existingRuntime || !!existingPersistentDisk) && h(ButtonOutline, {
          ...
          Utils.cond(
            ...
            [!existingRuntime && !!existingPersistentDisk, () => 'Delete Persistent 
Disk'],
            () => 'Delete Environment'
          )
        ]),
    ...
    return h(Fragment, [
        ...
        div({ style: { padding: '1.5rem', overflowY: 'auto', flex: 'auto' } }, [
          ...
          !!isPersistentDisk && h(PersistentDiskSection, { diskExists: 
!!existingPersistentDisk, computeConfig, updateComputeConfig, 
handleLearnMoreAboutPersistentDisk }),
          isGce(runtimeType) && !isPersistentDisk && div({ style: { 
...computeStyles.whiteBoxContainer, marginTop: '1rem' } }, [
            div([
              'Time to upgrade your cloud environment. Terra’s new persistent disk 
feature will safeguard your work and data. ',
              h(Link, { onClick: handleLearnMoreAboutPersistentDisk }, ['Learn more 
about Persistent disks and where your disk is mounted'])
            ]),
            h(ButtonOutline, {
              style: { marginTop: '1rem' },
              tooltip: 'Upgrade your environment to use a persistent disk. This will 
require a one-time deletion of your current built-in disk, but after that your data 
will be stored and preserved on the persistent disk.',
...
cluster
         resources and facilities, such as the YARN resource manager, the Hadoop 
Distributed
         File System (HDFS), MapReduce, and Spark.`
      ]),
      h(SparkInterface, { sparkInterface: sparkInterfaces.yarn, namespace, name, 
onDismiss }),
      h(SparkInterface, { sparkInterface: sparkInterfaces.appHistory, namespace, name, 
onDismiss }),
      h(SparkInterface, { sparkInterface: sparkInterfaces.sparkHistory, namespace, 
name, onDismiss }),
      h(SparkInterface, { sparkInterface: sparkInterfaces.jobHistory, namespace, name, 
onDismiss })
    ])
  }
