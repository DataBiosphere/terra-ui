...
export const ComputeModalBase = ({
  ...
  const applyChanges = _.flow(
    ...
    if (shouldDeletePersistentDisk && !hasAttachedDisk()) {
      await Ajax().Disks.diskV1().disk(googleProject, currentPersistentDiskDetails.name).delete()
    }
    ...
    if (shouldUpdateRuntime || shouldCreateRuntime) {
      ...
      if (shouldCreateRuntime) {
        ...
        if (shouldUpdatePersistentDisk) {
          await Ajax().Disks.diskV1().disk(googleProject, currentPersistentDiskDetails.name).update(desiredPersistentDisk.size)
        }
    ...
    const doUseOnMount = _.flow(
      ...
      const [currentRuntimeDetails, newLeoImages, currentPersistentDiskDetails] = await Promise.all([
        ...
        Ajax()
          ...
          .then(r => r.json()),
        currentDisk ? Ajax().Disks.diskV1().disk(currentDisk.googleProject, currentDisk.name).details() : null
      ])
