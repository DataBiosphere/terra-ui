...
import { getAppType, getToolsToDisplay, isPauseSupported, isToolLabelAnApp, tools } from 'src/pages/workspaces/workspace/analysis/tool-utils'
...
export const CloudEnvironmentModal = ({
  ...
  const executeAndRefresh = async (toolLabel, promise) => {
    try {
      setBusy(true)
      await promise
      await isToolLabelAnApp(toolLabel) ? refreshApps() : refreshRuntimes()
    } catch (error) {
  ...
  const startApp = toolLabel => Utils.cond([isToolLabelAnApp(toolLabel), () => {
  ...
  const stopApp = toolLabel => Utils.cond([isToolLabelAnApp(toolLabel), () => {
    const { appName, cloudContext } = currentApp(toolLabel)
    ...
    switch (status) {
      ...
      case 'Error':
        return h(RuntimeIcon, {
          ...
          onClick: () => {
            Utils.cond(
              [isToolLabelAnApp(toolLabel), () => setErrorAppId(currentApp(toolLabel)?.appName)],
  ...
  const isCloudEnvModalDisabled = toolLabel => Utils.cond(
    [isToolLabelAnApp(toolLabel), () => !canCompute || busy || (toolLabel === tools.Galaxy.label && isCurrentGalaxyDiskDetaching(apps)) || getIsAppBusy(currentApp(toolLabel))],
    [Utils.DEFAULT, () => {
  ...
  const getToolLaunchClickableProps = toolLabel => {
    ...
    const isToolBusy = isToolLabelAnApp(toolLabel) ?
