...
import { PersistentDiskSection } from './persistent-disk-controls'


// const AnySelect = Select as typeof Select<any>
...
export const ComputeModalBase = ({
  ...
  const [viewMode, setViewMode] = useState<string | undefined>(undefined)
  ...
  const getValidCpuGpuConfig = machineType => {
    ...
    // TODO: Add type for GPU. Why is one of types possible number?
    // @ts-expect-error
  ...
  const makeImageInfo = style => {
    ...
    // TODO: Will need to cast selected image to a LeoImage | undefined once we have a LeoImage type
    // @ts-expect-error
  ...
  const sendCloudEnvironmentMetrics = () => {
    ...
    Ajax().Metrics.captureEvent(Events[metricsEvent], {
      ...
      desiredRuntime_costPerHour: desiredRuntime && getGoogleRuntimeConfigCost(getPendingRuntimeConfig(), getPendingDisk()),
