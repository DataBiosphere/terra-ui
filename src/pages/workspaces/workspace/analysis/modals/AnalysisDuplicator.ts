...
export const AnalysisDuplicator = ({ destroyOld = false, fromLauncher = false, printName, toolLabel, workspaceInfo, onDismiss, onSuccess }: AnalysisDuplicatorProps) => {
  const [newName, setNewName] = useState<DisplayName | string>('')
  const { analyses }: { analyses: AnalysisFile[] } = useAnalysisFiles()
    ...
    okButton: h(ButtonPrimary, {
      ...
      onClick: withErrorReportingInModal(`Error ${destroyOld ? 'renaming' : 'copying'} analysis`, onDismiss, async () => {
        ...
        if (destroyOld) {
          await rename()
          // @ts-expect-error
          await Ajax().Metrics.captureEvent(Events.notebookRename, {
            oldName: printName,
            newName,
          ...
          await duplicate()
          // @ts-expect-error
          await Ajax().Metrics.captureEvent(Events.notebookCopy, {
            oldName: printName,
            newName,
            fromWorkspaceNamespace: workspaceInfo.namespace,
            fromWorkspaceName: workspaceInfo.name,
            toWorkspaceNamespace: workspaceInfo.namespace,
            toWorkspaceName: workspaceInfo.name
          })
        }
