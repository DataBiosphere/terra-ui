...
import {
  AppToolLabel,
  appToolLabels,
  appTools, getToolsToDisplayForCloudProvider, isAppToolLabel, isPauseSupported, isRuntimeToolLabel,
  isSettingsSupported,
...
export const CloudEnvironmentModal = ({
  ...
  const renderComputeModal = (tool: Tool) => h(ComputeModalBase, {
  ...
  const renderAppModal = (appModalBase: Component<any>, appMode: ToolLabel) => h(appModalBase, {
  ...
  const renderDefaultPage = () => div({ style: { display: 'flex', flexDirection: 'column', flex: 1 } },
    _.map((tool: Tool) => renderToolButtons(tool.label, cloudProvider))(
  ...
  const isLaunchSupported = (toolLabel: AppToolLabel): boolean => {
    const toolLaunchable = Object.values(tools).find(tool => tool.label === toolLabel)
    return toolLaunchable === undefined ? false : toolLaunchable.isLaunchUnsupported !== true
  }
  ...
  const executeAndRefresh = async (toolLabel: ToolLabel, promise: Promise<any>): Promise<void> => {
    try {
      setBusy(true)
      await promise
  ...
  const startApp = (toolLabel: ToolLabel): void => {
    if (isAppToolLabel(toolLabel)) {
      ...
      currentRuntime && executeAndRefresh(toolLabel,
  ...
  const stopApp = (toolLabel: ToolLabel): void => {
    if (isAppToolLabel(toolLabel)) {
      const { appName, cloudContext } = currentApp(toolLabel)!
      executeAndRefresh(toolLabel,
  ...
  const defaultIcon = (toolLabel: ToolLabel): ReactElement<any> => isPauseSupported(toolLabel) ? h(RuntimeIcon, {
    shape: 'pause',
    toolLabel,
    disabled: true,
  ...
  const renderStatusClickable = (toolLabel: ToolLabel): ReactElement<any> => {
    if (toolLabel === currentRuntimeTool) {
      ...
      const iconClickable = defaultIcon(toolLabel)
      return iconClickable ? iconClickable : h(div)
    }
  }

  const getIconFromStatus = (toolLabel: ToolLabel, status: LeoRuntimeStatus|AppStatus): ReactElement<any> => {
  ...
  const getToolIcon = (toolLabel: ToolLabel): string => Utils.switchCase(toolLabel,
  ...
  const isCloudEnvModalDisabled = (toolLabel: ToolLabel): boolean => {
    if (isAppToolLabel(toolLabel)) {
      ...
      return !currentRuntime || !runtime || !canCompute || busy || (currentRuntime && getIsRuntimeBusy(currentRuntime))
    }
    return true
  }
  ...
  const getToolLaunchClickableProps = (toolLabel: ToolLabel, cloudProvider: CloudProvider): ClickableProps => {
    ...
    const app = toolIsApp && currentApp(toolLabel)
    ...
    const isToolBusy = app && toolIsApp ?
  ...
  const renderToolButtons = (toolLabel: AppToolLabel, cloudProvider: CloudProvider): ReactElement<any> => {
  ...
  const modalBody: ReactElement<any> = h(Fragment, [
    h(TitleBar, {
      id: titleId,
