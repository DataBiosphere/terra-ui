...
export const CloudEnvironmentModal = ({
  ...
  const renderComputeModal = (tool: Tool) =>
    h(ComputeModalBase, {
      // TODO: is the tool type too general?
      workspace,
      tool,
      currentRuntime,
      currentDisk,
  ...
  const renderAppModal = (appModalBase: Component<any>) =>
    h(appModalBase, {
      workspace,
      apps,
      appDataDisks,
  ...
  const isLaunchSupported = (toolLabel: AppToolLabel) =>
    _.find((tool: Tool) => tool.label === toolLabel)(tools)!.isLaunchUnsupported;
  ...
  const isCloudEnvModalDisabled = (toolLabel: ToolLabel) => {
    if (isAppToolLabel(toolLabel)) {
      ...
      const runtime =
        currentRuntime &&
        currentRuntimeTool &&
        isRuntimeToolLabel(currentRuntimeTool) &&
      ...
      return runtime ? false : !canCompute || busy || (currentRuntime && getIsRuntimeBusy(currentRuntime));
    }
  };
  ...
  const getToolLaunchClickableProps = (toolLabel: ToolLabel, cloudProvider: CloudProvider): ClickableProps => {
    ...
    const isDisabled =
      ...
      busy ||
      isToolBusy ||
      (toolIsApp && !isLaunchSupported(toolLabel)) ||
    ...
    return h(Fragment, [
      ...
      div({ style: toolPanelStyles }, [
        ...
        div({ style: toolButtonDivStyles }, [
          isSettingsSupported(toolLabel, cloudProvider) &&
            h(
              Clickable,
              {
  ...
  const getGCPView = (): ReactElement<any> =>
    Utils.switchCase(
      viewMode,
  ...
  const getAzureView = (): ReactElement<any> =>
    Utils.switchCase(viewMode, [runtimeToolLabels.JupyterLab, renderAzureModal], [Utils.DEFAULT, renderDefaultPage]);

  const getView = (): ReactElement<any> =>
    Utils.switchCase(cloudProvider, [cloudProviderTypes.GCP, getGCPView], [cloudProviderTypes.AZURE, getAzureView]);

  const width: number = Utils.switchCase(
    viewMode,
  ...
  const modalBody = h(Fragment, [
    h(TitleBar, {
      ...
      titleStyles: _.merge(viewMode === undefined ? {} : { display: 'none' }, {
