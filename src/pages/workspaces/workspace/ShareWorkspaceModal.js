...
export default ajaxCaller(class ShareWorkspaceModal extends Component {
  ...
  async save() {
    ...
    try {
      ...
      !!numAdditions && Ajax().Metrics.captureEvent(Events.workspaceShare, { ...extractWorkspaceDetails(this.props.workspace), numAdditions, success: true })
      onDismiss()
    } catch (error) {
      !!numAdditions && Ajax().Metrics.captureEvent(Events.workspaceShare, { ...extractWorkspaceDetails(this.props.workspace), numAdditions, success: false })
