...
const useCloudEnvironmentPolling = (googleProject, workspace) => {
  ...
  const load = async maybeStale => {
    try {
      ...
      const cloudEnvFilters = _.pickBy(l => !_.isUndefined(l), saturnWorkspaceVersion === 'v1' ?
        { creator: getUser().email, googleProject } :
        { creator: getUser().email, saturnWorkspaceName, saturnWorkspaceNamespace })
