...
const MetadataDetailsComponent = ({ dataObj, name }) => {
  return h(Fragment, [
    ...
    div({ style: { display: "flex", width: "100%", flexWrap: "wrap" } }, [
      ...
      div({ style: styles.attributesColumn }, [h3({ style: styles.headers }, ["Last Updated"]), formatDatasetTime(dataObj["dct:modified"])]),
      ...
      div({ style: styles.attributesColumn }, [h3({ style: styles.headers }, ["Owner"]), dataObj["TerraDCAT_ap:hasOwner"]]),
      div({ style: styles.attributesColumn }, [
    ...
    !!workspaceName &&
      div({ style: { marginBottom: "1rem" } }, [
        ...
        h(
          ...
          [icon("folderSolid", { size: 18, style: { marginRight: 10, color: styles.access.controlled } }), workspaceName]
        ),
      ]),
  ...
  return h(Fragment, [
    div({ style: { ...styles.content, width: 300, flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "center" } }, [
      ...
      div({ style: { backgroundColor: "white", padding: 20, paddingTop: 0, width: "100%", border: "2px solid #D6D7D7", borderRadius: 5 } }, [
        div([h3(["Access type"]), div({ style: { display: "flex", alignItems: "flex-start" } }, [h(DatasetAccess, { dataset: dataObj })])]),
        div([h3({ style: styles.headers }, ["Donor size"]), div([_.getOr(0, "counts.donors", dataObj).toLocaleString()])]),
        div([h3({ style: styles.headers }, ["Sample size"]), div([_.getOr(0, "counts.samples", dataObj).toLocaleString()])]),
        div([h3({ style: styles.headers }, ["Data modality"]), div([_.join(", ", getDataModalityListFromDataset(dataObj))])]),
        div([h3({ style: styles.headers }, ["Assay category"]), div([_.join(", ", getAssayCategoryListFromDataset(dataObj))])]),
        div([
