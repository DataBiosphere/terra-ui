...
describe('Step', () => {
  ...
  it('has a more intense border and lighter background when active', () => {
    ...
    const rgbToTotalNumber = (rgb: string) => sum(rgb.substring(rgb.indexOf('(') + 1, rgb.indexOf(')')).split(',').map(s => parseInt(s)))
    ...
    expect(parseInt(activeStyle.borderColor.substring(1), 16))
