...
export const CreateNamedProjectStep = ({ isActive, ...props }: CreateProjectStepProps) => {
  ...
  return h(Step, { isActive }, [
    ...
    h(StepFields, { style: { flexDirection: 'column' } }, [
      h(StepFieldLegend, { style: { width: '90%' } }, [
        'Name your Terra Billing Project'
      ]),
      div({ style: rowStyle }, [
        div({ style: { ...columnEntryStyle(true), marginBottom: '1.25rem' } },
          [h(LabeledField,
            { label: 'Billing project name', formId: nameInputId, required: true }, [
              ValidatedInput({
            ...
            ]), !props.projectNameErrors && div({ style: hintStyle },
            [formHint('Name must be unique and cannot be changed')])]),
