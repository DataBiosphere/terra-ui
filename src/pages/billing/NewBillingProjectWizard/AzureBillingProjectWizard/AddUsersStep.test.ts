...
export const getNoUsersRadio = () => screen.getByLabelText("I don't have additional owners or users to add at this time")
...
describe('AddUsersStep', () => {
  ...
  it('Disables and clears the input fields based on radio button selection', async () => {
    ...
    const mockAddUsersOrOwners = jest.fn(addUsers => {
      renderResult.rerender(h(AddUsersStep, {
        ...defaultProps, onAddUsersOrOwners: mockAddUsersOrOwners, addUsersOrOwners: addUsers
      }))
    })
    const renderResult = render(
      h(AddUsersStep, { ...defaultProps, onAddUsersOrOwners: mockAddUsersOrOwners })
    )
