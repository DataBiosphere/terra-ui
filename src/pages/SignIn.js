...
const SignIn = () => {
  return h(HeroWrapper, { showMenu: false, showDocLink: true }, [
    div({ style: { maxWidth: 600 } }, [
      ...
      div({ style: { fontSize: 36, marginBottom: '2rem' } }, [`${getAppName({ capitalInitial: true })} requires a Google Account.`]),
      div({ style: { fontSize: 16, lineHeight: 1.5, marginBottom: '2rem' } }, [
        `${getAppName({ capitalInitial: true })} uses your Google account. Once you have signed in and completed the user profile registration step, you can start using ${getAppName()}.`
      ]),
      h(SignInButton),
