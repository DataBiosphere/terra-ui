...
import { isRuntimeToolLabel, RuntimeToolLabel, runtimeToolLabels } from 'src/analysis/utils/tool-utils';
import Collapse from 'src/components/Collapse';
import { ButtonPrimary, ButtonSecondary, IdContainer, Link, RadioButton, spinnerOverlay } from 'src/components/common';
...
import { ChoiceButton, isProtectedWorkspace, styles } from 'src/pages/ImportData';
  ...
  div({ style: styles.card }, [
    ...
    url &&
      div({ style: { fontSize: 16 } }, [
        'From: ',
        referrer !== undefined && referrer ? referrer : new URL(url).hostname,
      ]),
    div(
      { style: { marginTop: '1rem' } },
      requireProtectedWorkspace
        ? [
            icon('warning-standard', { size: 15, style: { marginRight: '0.25rem' } }),
    ...
    h(Fragment, [
      ...
      requireProtectedWorkspace &&
        div({ style: { marginTop: '0.5rem', lineHeight: '1.5' } }, [
          icon('info-circle', { size: 15, style: { marginRight: '0.25rem' } }),
...
export const importAnalysisFile = async (
  namespace: string,
  name: string,
  url: string,
  filename: string,
  ...
  if (!isRuntimeToolLabel(toolLabel)) {
    throw new Error(`${toolLabel} is not a valid analysis runtime tool label: ${Object.values(runtimeToolLabels)}`);
  }
  ...
  await AnalysisProvider.createAnalysis(workspaceDetails.workspace, filename, toolLabel, fileContents);
...
const ImportAnalysis = () => {
  const {
    query: { url, ad, wid, template, referrer, requireProtected, filename, toolLabel },
  } = Nav.useRoute();
  ...
  const [title, header] = ['Import Analysis', `Analysis (${toolLabel})`];
  ...
  const onImport = _.flow(
    ...
    await importAnalysisFile(namespace, name, url, filename, toolLabel);
  ...
  return h(FooterWrapper, [
    h(TopBar, { title, href: undefined, children: undefined }),
    div({ role: 'main', style: styles.container as React.CSSProperties }, [
      img({
