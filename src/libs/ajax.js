...
const fetchSam = async (path, options) => {
  return fetchOk(`${(await getConfig()).samUrlRoot}/${path}`, addAppIdentifier(options))
}
...
const fetchRawls = async (path, options) => {
  return fetchOk(`${(await getConfig()).rawlsUrlRoot}/api/${path}`, addAppIdentifier(options))
}

const fetchLeo = async (path, options) => {
  return fetchOk(`${(await getConfig()).leoUrlRoot}/${path}`, options)
}

const fetchDockstore = async (path, options) => {
  return fetchOk(`${(await getConfig()).dockstoreUrlRoot}/${path}`, options)
}
// %23 = '#', %2F = '/'
...
const fetchAgora = async (path, options) => {
  return fetchOk(`${(await getConfig()).agoraUrlRoot}/api/v1/${path}`, addAppIdentifier(options))
}

const fetchOrchestration = async (path, options) => {
  return fetchOk(`${(await getConfig()).orchestrationUrlRoot}/${path}`, addAppIdentifier(options))
}
...
const User = signal => ({
  ...
  getStatus: async () => {
    return instrumentedFetch(`${(await getConfig()).samUrlRoot}/register/user/v2/self/info`, _.mergeAll([authOpts(), { signal }, appIdentifier]))
  },
  ...
  getTosAccepted: async () => {
    const url = `${(await getConfig()).tosUrlRoot}/user/response?${qs.stringify(tosData)}`
  ...
  acceptTos: async () => {
    await fetchOk(
      `${(await getConfig()).tosUrlRoot}/user/response`,
  ...
  notebook: (namespace, bucket, name) => {
    ...
    return {
      preview: async () => {
        ...
        return fetchOk(`${(await getConfig()).calhounUrlRoot}/api/convert`,
    ...
    return {
      create: async clusterOptions => {
        const body = _.merge(clusterOptions, {
          ...
          defaultClientId: (await getConfig()).googleClientId,
          userJupyterExtensionConfig: {
            nbExtensions: {
              'saturn-iframe-extension':
                `${window.location.hostname === 'localhost' ? (await getConfig()).devUrlRoot : window.location.origin}/jupyter-iframe-extension.js`
            },
            serverExtensions: {},
            combinedExtensions: {}
          }
...
const Martha = signal => ({
  call: async uri => {
    return fetchOk((await getConfig()).marthaUrlRoot,
