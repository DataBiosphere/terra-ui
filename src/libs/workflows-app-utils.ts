...
export const resolveRunningCromwellAppUrl = (apps: ListAppResponse[], currentUser: string) => {
  ...
  const healthyState = appStatuses.running.status;
  const cromwellAppType = appToolLabels.CROMWELL;
  ...
  const filteredApps: ListAppResponse[] = apps.filter(
    (app) => app.appType === cromwellAppType && app.status === healthyState && app.auditInfo.creator === currentUser
  );
