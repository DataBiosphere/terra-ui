const allProviders = [
  { key: 'fence', name: 'NHLBI BioData Catalyst Framework Services', expiresAfter: 30, short: 'NHLBI' },
  { key: 'dcf-fence', name: 'NCI CRDC Framework Services', expiresAfter: 15, short: 'NCI' },
  { key: 'anvil', name: 'NHGRI AnVIL Data Commons Framework Services', expiresAfter: 30, short: 'NHGRI' },
  { key: 'kids-first', name: 'Kids First DRC', expiresAfter: 30, short: 'KidsFirst' }
]

export default allProviders
