...
)(({ name: workspaceName, analysesData: { runtimes, refreshRuntimes }, application, workspace: { azureContext, workspace }, analysisName }, _ref) => {
    ...
    const computeIframeSrc = withErrorReporting('Error loading application iframe')(async () => {
      ...
      const url = await Utils.switchCase(
        ...
        [
          Utils.DEFAULT,
          () =>
            console.error(`Expected ${application} to be one of terminal, spark, ${runtimeToolLabels.RStudio}, or ${runtimeToolLabels.JupyterLab}.`),
        ]
      );
