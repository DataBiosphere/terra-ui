...
async function serviceAccountSetup() {
  ...
  // serviceAccount name:
  const accountId = 'a' + makeUniqueId()
  ...
  var sa = await google.iam({ version: 'v1', auth: jwtClient })
    .projects
    .serviceAccounts
    .create({ name: 'projects/aj-saturn-tests', resource: { accountId: accountId } })
  ...
  var sa = await sa._options.auth.createScoped(['https://www.googleapis.com/auth/userinfo.profile', 'https://www.googleapis.com/auth/userinfo.email'])
  ...
  const token = await sa._options.auth.createScoped(
    ['https://www.googleapis.com/auth/userinfo.profile', 'https://www.googleapis.com/auth/userinfo.email']).getAccessToken().then(res => {
    console.log(res)
...
function makeUniqueId() {
  const id = Date.now().toString() + '-' + Math.random().toString().substring(2, 12)
  return id
}
...
describe('Google', () => {
  ...
  it('hello - real google login', async () => {
    ...
    console.log('TEST A')
    await expect(page).toMatch('Terra')

    // await expect(page).toMatch('Create A New Workspace')
    console.log('TEST B')
    return expect(page).toMatch('New Workspace')
