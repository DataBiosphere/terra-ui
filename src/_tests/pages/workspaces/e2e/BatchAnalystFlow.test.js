...
import { getAccessToken, wait, generateUUID, setupTest, cleanupTest, createWorkspace, waitForNetworkIdle } from 'src/_tests/test-utils'
...
describe('Google', () => {
  ...
  it('create a workspace and launch a batch analysis (workflow)', async () => {
    ...
    await expect(page).toClick('[datatestid="workflow-link-echo_strings"]')
    ...
    await Promise.all([
      expect(page).toClick('[datatestid="saveWorkflowButton"]'),
    ...
    await expect(page).toClick('[datatestid="data-tab"]')
    await expect(page).toClick('a', { text: 'participant (1)' })
  ...
  }, 300 * 1000) // this is the max timeout for any individual step (?)
})
